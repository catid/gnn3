...
from gnn3.models.prototype_defer import SpecialistPrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_specialist_max", "prototype_specialist_gate")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--headroom-prototypes", type=int, default=6)
    parser.add_argument("--residual-prototypes", type=int, default=6)
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_specialist_defer",
...
def _positive_masks(meta: pd.DataFrame) -> tuple[torch.Tensor, torch.Tensor]:
    stable = meta["stable_positive_v2_case"].to_numpy(copy=True)
    headroom = meta["high_headroom_near_tie_case"].to_numpy(copy=True)
    headroom_positive = stable & headroom
    residual_positive = stable & ~headroom
    return (
        torch.as_tensor(headroom_positive, dtype=torch.bool),
        torch.as_tensor(residual_positive, dtype=torch.bool),
    )
...
def _fit_head(
    ...
    headroom_positive: torch.Tensor,
    residual_positive: torch.Tensor,
    ...
    *,
    ...
    y = target.to(device)
    w = weights.to(device)
    headroom_mask = headroom_positive.to(device)
    residual_mask = residual_positive.to(device)
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    for variant, group in stable.groupby("variant", sort=False):
        axes[0].plot(group["budget_pct"], group["correction_rate"], marker="o", label=variant)
    ...
    axes[0].set_title("Specialist Prototype Stable-Positive Recovery")
    axes[1].set_title("Specialist Prototype Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    headroom_positive, residual_positive = _positive_masks(train_meta)
    ...
    for variant in VARIANTS:
        model = _fit_head(
            ...
            headroom_prototypes=args.headroom_prototypes,
            residual_prototypes=args.residual_prototypes,
