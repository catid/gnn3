...
from gnn3.models.prototype_defer import RiskPriorRegimeMemoryBlendPrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_risk_prior_regime_memory", "prototype_risk_prior_regime_memory_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_risk_prior_regime_memory_blend_defer",
...
def _fit_head(
    ...
    *,
    ...
    use_risk = variant == "prototype_risk_prior_regime_memory_hybrid"
    model = RiskPriorRegimeMemoryBlendPrototypeDeferHead(
    ...
    for _ in range(epochs):
        ...
        logits, regime_logits = model.forward_with_regime(x, risk)
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Risk-Prior Regime Memory Precision vs Coverage")
    axes[1].set_title("Risk-Prior Regime Memory Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        for split_name, eval_meta, eval_features, eval_risk in eval_parts:
            ...
            with torch.no_grad():
                scores, regime_logits = model.forward_with_regime(eval_norm, risk_norm_eval)
            ...
            model_metadata[variant].append(
                {
                    ...
                    "use_risk_branch": variant == "prototype_risk_prior_regime_memory_hybrid",
                }
            )
