...
from gnn3.models.prototype_defer import PrototypeTriageDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("triage_memory", "triage_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--neutral-prototypes", type=int, default=12)
    parser.add_argument("--harmful-prototypes", type=int, default=4)
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_triage_defer",
...
def _class_labels(meta: pd.DataFrame) -> torch.Tensor:
    labels = np.full(len(meta), 1, dtype=np.int64)
    labels[meta["harmful_teacher_bank_case"].to_numpy(copy=True)] = 2
    labels[meta["stable_positive_v2_case"].to_numpy(copy=True)] = 0
    return torch.as_tensor(labels, dtype=torch.long)


def _weights(meta: pd.DataFrame, labels: torch.Tensor) -> torch.Tensor:
    values = np.full(len(meta), 0.15, dtype=np.float32)
    values[labels.numpy() == 1] = 0.10
    values[labels.numpy() == 2] = 2.0
    pos_mask = labels.numpy() == 0
    values[pos_mask] = (
        3.0
...
def _fit_head(
    ...
    *,
    ...
    neutral_prototypes: int,
    harmful_prototypes: int,
    hidden_dim: int,
    device: torch.device,
) -> PrototypeTriageDeferHead:
    use_risk = variant == "triage_hybrid"
    model = PrototypeTriageDeferHead(
        feature_dim=train_features.size(1),
        risk_dim=train_risk.size(1),
        prototype_dim=prototype_dim,
        positive_prototypes=positive_prototypes,
        neutral_prototypes=neutral_prototypes,
        harmful_prototypes=harmful_prototypes,
    ...
    y = labels.to(device)
    w = weights.to(device)
    class_counts = torch.bincount(labels, minlength=3).float().clamp(min=1.0).to(device)
    class_weight = class_counts.sum() / class_counts
    positive_mask = y == 0
    neutral_mask = y == 1
    harmful_mask = y == 2
    for _ in range(epochs):
        optimizer.zero_grad(set_to_none=True)
        logits = model.class_logits(x, risk if use_risk else None)
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Prototype-Triage Precision vs Coverage")
    axes[1].set_title("Prototype-Triage Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    labels = _class_labels(train_meta)
    weights = _weights(train_meta, labels)
    ...
    for variant in VARIANTS:
        model = _fit_head(
            ...
            neutral_prototypes=args.neutral_prototypes,
            harmful_prototypes=args.harmful_prototypes,
        ...
        for split_name, eval_meta, eval_features, eval_risk in eval_payloads:
            ...
            with torch.no_grad():
                scores = model(eval_norm, risk_norm_eval if variant == "triage_hybrid" else None).numpy()
