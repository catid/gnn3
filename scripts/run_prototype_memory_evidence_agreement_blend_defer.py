...
from gnn3.models.prototype_defer import MemoryEvidenceAgreementBlendPrototypeDeferHead
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_memory_evidence_agreement_blend_defer",
...
def _fit_head(
    ...
    *,
    ...
    use_risk = variant == "prototype_memory_evidence_blend_hybrid"
    model = MemoryEvidenceAgreementBlendPrototypeDeferHead(
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Memory-Evidence Blend Precision vs Coverage")
    axes[1].set_title("Memory-Evidence Blend Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        for split_name, eval_meta, eval_features, eval_risk in eval_parts:
            ...
            with torch.no_grad():
                scores = model(eval_norm, risk_norm_eval if variant == "prototype_memory_evidence_blend_hybrid" else None).numpy()
            model_metadata[variant].append(
                {
                    ...
                    "use_risk_branch": variant == "prototype_memory_evidence_blend_hybrid",
                }
            )
