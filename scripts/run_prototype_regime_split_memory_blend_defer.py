...
from gnn3.models.prototype_defer import RegimeSplitMemoryBlendPrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_regime_split_memory", "prototype_regime_split_memory_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_regime_split_memory_blend_defer",
...
def _fit_head(
    ...
    *,
    ...
    use_risk = variant == "prototype_regime_split_memory_hybrid"
    model = RegimeSplitMemoryBlendPrototypeDeferHead(
    ...
    for _ in range(epochs):
        ...
        logits, regime_logits = model.forward_with_regime(x, risk if use_risk else None)
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Regime-Split Memory Precision vs Coverage")
    axes[1].set_title("Regime-Split Memory Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        for split_name, eval_meta, eval_features, eval_risk in eval_parts:
            ...
            with torch.no_grad():
                scores, regime_logits = model.forward_with_regime(
                    eval_norm,
                    risk_norm_eval if variant == "prototype_regime_split_memory_hybrid" else None,
                )
            ...
            model_metadata[variant].append(
                {
                    ...
                    "use_risk_branch": variant == "prototype_regime_split_memory_hybrid",
                }
            )
