...
from gnn3.models.prototype_defer import RiskVetoRegimeMemoryBlendPrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_risk_veto_regime_memory", "prototype_risk_veto_regime_memory_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--regime-loss-weight", type=float, default=0.10)
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_risk_veto_regime_memory_blend_defer",
...
def _attach_teacher_bank(meta: pd.DataFrame, teacher_bank: pd.DataFrame) -> pd.DataFrame:
    merge_cols = [*KEY_COLS, "stable_positive_v2_case", "harmful_teacher_bank_case"]
...
def _regime_targets(meta: pd.DataFrame) -> torch.Tensor:
    positive = meta["stable_positive_v2_case"].to_numpy(copy=True).astype(bool)
    headroom = positive & meta["high_headroom_near_tie_case"].to_numpy(copy=True).astype(bool)
    residual = positive & (~headroom) & meta["baseline_error_hard_near_tie_case"].to_numpy(copy=True).astype(bool)
    target = np.zeros(len(meta), dtype=np.int64)
    target[headroom] = 1
    target[residual] = 2
    return torch.as_tensor(target, dtype=torch.long)


def _veto_targets(meta: pd.DataFrame) -> torch.Tensor:
    positive = meta["stable_positive_v2_case"].to_numpy(copy=True).astype(bool)
    headroom = positive & meta["high_headroom_near_tie_case"].to_numpy(copy=True).astype(bool)
    residual = positive & (~headroom) & meta["baseline_error_hard_near_tie_case"].to_numpy(copy=True).astype(bool)
    target = np.zeros((len(meta), 2), dtype=np.float32)
    target[headroom, 0] = 1.0
    target[residual, 1] = 1.0
...
def _fit_head(
    ...
    *,
    ...
    epochs: int,
    lr: float,
    regime_loss_weight: float,
    veto_loss_weight: float,
    prototype_dim: int,
    ...
    use_risk = variant == "prototype_risk_veto_regime_memory_hybrid"
    model = RiskVetoRegimeMemoryBlendPrototypeDeferHead(
    ...
    y = target.to(device)
    regime_y = regime_target.to(device)
    veto_y = veto_target.to(device)
    w = weights.to(device)
    ...
    for _ in range(epochs):
        ...
        logits, regime_logits, veto_logits = model.forward_with_regime(x, risk if use_risk else None)
        ...
        if bool(positive.any()):
            regime_loss = F.cross_entropy(regime_logits[positive], regime_y[positive], reduction="none")
            regime_weights = w[positive] / w[positive].mean().clamp(min=1e-6)
            loss = loss + regime_loss_weight * (regime_loss * regime_weights).mean()
        if use_risk:
            veto_loss = F.binary_cross_entropy_with_logits(veto_logits, veto_y, reduction="none").mean(dim=1)
            loss = loss + veto_loss_weight * (veto_loss * w).mean()
        loss.backward()
        optimizer.step()
    return model.cpu()
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Risk-Veto Regime Memory Precision vs Coverage")
    axes[1].set_title("Risk-Veto Regime Memory Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    teacher_bank = teacher_bank[[*KEY_COLS, "stable_positive_v2_case", "harmful_teacher_bank_case"]].drop_duplicates()
    ...
    eval_parts: list[tuple[str, pd.DataFrame, torch.Tensor, torch.Tensor]] = []
    for cache_path, metadata_path in zip(args.eval_caches, args.eval_metadata, strict=True):
        ...
        features, risk = _feature_sets(cache, meta)
        eval_parts.append((Path(cache_path).stem, meta, features, risk))

    all_summary: list[pd.DataFrame] = []
    all_decisions: list[pd.DataFrame] = []
    model_metadata: dict[str, list[dict[str, object]]] = {}
    device = torch.device(args.device)

    hard_negative_mask = torch.as_tensor(
        train_meta["harmful_teacher_bank_case"].to_numpy(copy=True) | (~train_meta["stable_positive_v2_case"].to_numpy(copy=True)),
        dtype=torch.bool,
    )
    target = _labels(train_meta)
    regime_target = _regime_targets(train_meta)
    veto_target = _veto_targets(train_meta)
    weights = _weights(train_meta, target)

    for variant in VARIANTS:
        model_metadata[variant] = []
        for split_name, eval_meta, eval_features, eval_risk in eval_parts:
            train_norm, eval_norm = _normalize(train_features, eval_features)
            risk_norm_train, risk_norm_eval = _normalize(train_risk, eval_risk)
            model = _fit_head(
                train_norm,
                risk_norm_train,
                target,
                ...
                veto_loss_weight=args.veto_loss_weight,
            ...
            with torch.no_grad():
                scores, regime_logits, veto_logits = model.forward_with_regime(
                    eval_norm,
                    risk_norm_eval if variant == "prototype_risk_veto_regime_memory_hybrid" else None,
                )
                regime_probs = torch.softmax(regime_logits, dim=1)
                veto_masks = torch.sigmoid(veto_logits)
                scores = scores.numpy()
                regime_probs = regime_probs.numpy()
                veto_masks = veto_masks.numpy()
            model_metadata[variant].append(
                {
                    "split": split_name,
                    "score_min": float(scores.min()),
                    "score_max": float(scores.max()),
                    "score_mean": float(scores.mean()),
                    "base_prob_mean": float(regime_probs[:, 0].mean()),
                    "headroom_prob_mean": float(regime_probs[:, 1].mean()),
                    "residual_prob_mean": float(regime_probs[:, 2].mean()),
                    "headroom_veto_mean": float(veto_masks[:, 0].mean()),
                    "residual_veto_mean": float(veto_masks[:, 1].mean()),
                    "use_risk_branch": variant == "prototype_risk_veto_regime_memory_hybrid",
                }
            )
            for budget in args.coverage_budgets:
                summary_df, decision_df = _evaluate_budget(eval_meta, scores, budget_pct=budget, variant=variant, split=split_name)
    ...
    json_path.write_text(json.dumps(model_metadata, indent=2))
