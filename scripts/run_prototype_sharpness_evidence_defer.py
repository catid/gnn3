...
from gnn3.models.prototype_defer import SharpnessEvidenceAgreementPrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_sharpness_evidence", "prototype_sharpness_evidence_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_sharpness_evidence_defer",
...
def _fit_head(
    ...
    *,
    ...
    use_risk = variant == "prototype_sharpness_evidence_hybrid"
    model = SharpnessEvidenceAgreementPrototypeDeferHead(
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Sharpness Evidence Precision vs Coverage")
    axes[1].set_title("Sharpness Evidence Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        for split_name, eval_meta, eval_features, eval_risk in eval_parts:
            ...
            with torch.no_grad():
                scores = model(eval_norm, risk_norm_eval if variant == "prototype_sharpness_evidence_hybrid" else None).numpy()
            model_metadata[variant].append(
                {
                    ...
                    "use_risk_branch": variant == "prototype_sharpness_evidence_hybrid",
                }
            )
