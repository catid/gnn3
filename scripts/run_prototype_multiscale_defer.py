...
from gnn3.models.prototype_defer import MultiscalePrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_multiscale", "prototype_multiscale_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_multiscale_defer",
...
def _fit_head(
    ...
    *,
    ...
    use_risk = variant != "prototype_multiscale"
    model = MultiscalePrototypeDeferHead(
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Prototype-Multiscale Precision vs Coverage")
    axes[1].set_title("Prototype-Multiscale Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        for split_name, eval_meta, eval_features, eval_risk in eval_payloads:
            ...
            with torch.no_grad():
                scores = model(eval_norm, risk_norm_eval if variant != "prototype_multiscale" else None).squeeze(-1).numpy()
