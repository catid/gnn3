...
VARIANTS = ("candidate_memory", "candidate_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/candidate_prototype_defer",
...
def _feature_sets(cache: dict[str, torch.Tensor], metadata: pd.DataFrame) -> tuple[torch.Tensor, torch.Tensor]:
    pair = candidate_pair_features(cache, metadata).float()
...
def _fit_head(
    ...
    *,
    ...
    use_risk = variant == "candidate_hybrid"
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Candidate-Prototype Precision vs Coverage")
    axes[1].set_title("Candidate-Prototype Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        for split_name, eval_meta, eval_features, eval_risk in eval_payloads:
            ...
            with torch.no_grad():
                scores = model(eval_norm, risk_norm_eval if variant == "candidate_hybrid" else None).numpy()
