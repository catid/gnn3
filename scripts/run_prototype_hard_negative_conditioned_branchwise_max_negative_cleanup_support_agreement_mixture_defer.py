...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_hard_negative_conditioned_branchwise_max_negative_cleanup_support_agreement_mixture_defer",
...
def _search_keep_masks(
    ...
    *,
    ...
    with torch.no_grad():
        shared_utility = _prototype_utility(
            _shared_negative_logits(base_model, train_features),
    ...
    shared_order = torch.argsort(shared_utility, descending=True)
    ...
    for shared_keep_count in keep_counts:
        for dual_keep_count in keep_counts:
            shared_mask = torch.zeros(args.negative_prototypes, dtype=torch.bool)
            dual_mask = torch.zeros(args.negative_prototypes, dtype=torch.bool)
            shared_mask[shared_order[:shared_keep_count]] = True
            ...
            row = {
                "variant": variant,
                "shared_keep_count": shared_keep_count,
                "dual_keep_count": dual_keep_count,
                "shared_utility_min": float(shared_utility[shared_mask].min().item()),
...
def _plot(summary: pd.DataFrame, output_prefix: Path) -> None:
    ...
    ax.set_title("Hard-negative-conditioned branchwise-max negative-cleanup support agreement mixture")
