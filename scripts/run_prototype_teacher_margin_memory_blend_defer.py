...
from gnn3.models.prototype_defer import TeacherMarginMemoryBlendPrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_teacher_margin_memory", "prototype_teacher_margin_memory_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--gain-loss-weight", type=float, default=0.10)
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_teacher_margin_memory_blend_defer",
...
def _gain_target(meta: pd.DataFrame) -> torch.Tensor:
    gain = np.log1p(meta["best_safe_teacher_gain"].clip(lower=0.0).to_numpy(copy=True))
    return torch.as_tensor(gain, dtype=torch.float32)
...
def _fit_head(
    ...
    *,
    ...
    use_risk = variant == "prototype_teacher_margin_memory_hybrid"
    model = TeacherMarginMemoryBlendPrototypeDeferHead(
    ...
    for _ in range(epochs):
        ...
        logits, predicted_gain = model.forward_with_gain(x, risk if use_risk else None)
        ...
        if bool(positive.any()):
            gain_loss = F.smooth_l1_loss(predicted_gain[positive], gain_y[positive], reduction="none")
            gain_weights = w[positive] / w[positive].mean().clamp(min=1e-6)
            loss = loss + gain_loss_weight * (gain_loss * gain_weights).mean()
        loss.backward()
        optimizer.step()
    return model.cpu()
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Teacher-Margin Memory Precision vs Coverage")
    axes[1].set_title("Teacher-Margin Memory Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    gain_target = _gain_target(train_meta)
    ...
    for variant in VARIANTS:
        ...
        for split_name, eval_meta, eval_features, eval_risk in eval_parts:
            ...
            with torch.no_grad():
                logits, predicted_gain = model.forward_with_gain(
                    eval_norm,
                    risk_norm_eval if variant == "prototype_teacher_margin_memory_hybrid" else None,
                )
                scores = logits.numpy()
                gain_scores = predicted_gain.numpy()
            model_metadata[variant].append(
                {
                    ...
                    "gain_min": float(gain_scores.min()),
                    "gain_max": float(gain_scores.max()),
                    "gain_mean": float(gain_scores.mean()),
                    "use_risk_branch": variant == "prototype_teacher_margin_memory_hybrid",
                }
            )
