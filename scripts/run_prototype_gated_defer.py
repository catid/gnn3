...
from gnn3.models.prototype_defer import GatedPrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_gated_scale", "prototype_gated_offset")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_gated_defer",
...
def _fit_head(
    ...
    *,
    ...
    model = GatedPrototypeDeferHead(
        ...
        use_bias_branch=variant == "prototype_gated_offset",
    ).to(device)
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Gated Prototype Stable-Positive Recovery")
    axes[1].set_title("Gated Prototype Hard Near-Tie Delta Regret")
