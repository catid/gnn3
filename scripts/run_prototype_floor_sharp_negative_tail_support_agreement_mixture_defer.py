...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tail-shrink-scale", type=float, default=1.0)
    parser.add_argument("--adaptive-tail-shrink-scale", type=float, default=1.5)
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_floor_sharp_negative_tail_support_agreement_mixture_defer",
...
def _fit_head(
    ...
    *,
    ...
    adaptive_tail_shrink_scale: float,
    sharpness_center: float,
    sharpness_scale: float,
    device: torch.device,
) -> FloorSharpNegativeTailSupportAgreementMixturePrototypeDeferHead:
    use_risk = variant == "prototype_floor_sharp_negative_tail_support_agree_mix_hybrid"
    model = FloorSharpNegativeTailSupportAgreementMixturePrototypeDeferHead(
        ...
        adaptive_tail_shrink_scale=adaptive_tail_shrink_scale,
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Floor+Sharp Negative-Tail Precision vs Coverage")
    axes[1].set_title("Floor+Sharp Negative-Tail Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    for variant in VARIANTS:
        model = _fit_head(
            ...
            adaptive_tail_shrink_scale=args.adaptive_tail_shrink_scale,
