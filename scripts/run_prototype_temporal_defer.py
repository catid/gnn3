...
VARIANTS = ("temporal_scalar_hybrid", "temporal_probe_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_temporal_defer",
...
def _top2_margin(scores: torch.Tensor, valid_mask: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
...
def _temporal_features(cache: dict[str, torch.Tensor], metadata: pd.DataFrame, *, variant: str) -> torch.Tensor:
    per_step_scores = cache["per_step_selection_scores"].float()
    valid_mask = cache["candidate_mask"].bool().unsqueeze(1).expand(-1, per_step_scores.size(1), -1)
    top1, margins = _top2_margin(per_step_scores, valid_mask)
    switch_count = (top1[:, 1:] != top1[:, :-1]).float().sum(dim=1, keepdim=True)
    margin_span = (margins.max(dim=1).values - margins.min(dim=1).values).unsqueeze(1)
    margin_std = margins.std(dim=1, keepdim=True)
    scalar = torch.cat(
        [
            margins,
            margin_span,
            margin_std,
            (margins[:, -1] - margins[:, 0]).unsqueeze(1),
            switch_count / max(per_step_scores.size(1) - 1, 1),
        ],
        dim=1,
    )

    headroom = torch.as_tensor(metadata[["base_predicted_continuation_gap"]].to_numpy(copy=True), dtype=torch.float32)
    base_risk = torch.cat([margin_only_features(metadata), margin_regime_features(metadata), headroom], dim=1)
    if variant == "temporal_scalar_hybrid":
        return torch.cat([base_risk, scalar], dim=1)

    probes = cache["per_step_probe_features"].float()
    step01 = probes[:, 1] - probes[:, 0]
    step12 = probes[:, 2] - probes[:, 1]
    step02 = probes[:, 2] - probes[:, 0]
    drift = torch.cat(
        [
            torch.linalg.vector_norm(step01, dim=1, keepdim=True) / np.sqrt(probes.size(-1)),
            torch.linalg.vector_norm(step12, dim=1, keepdim=True) / np.sqrt(probes.size(-1)),
            torch.linalg.vector_norm(step02, dim=1, keepdim=True) / np.sqrt(probes.size(-1)),
            F.cosine_similarity(probes[:, 0], probes[:, 1], dim=1).unsqueeze(1),
            F.cosine_similarity(probes[:, 1], probes[:, 2], dim=1).unsqueeze(1),
            F.cosine_similarity(probes[:, 0], probes[:, 2], dim=1).unsqueeze(1),
        ],
        dim=1,
    )
    return torch.cat([base_risk, scalar, drift], dim=1)


def _feature_sets(cache: dict[str, torch.Tensor], metadata: pd.DataFrame) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    decision = decision_augmented_features(cache, metadata).float()
    return decision, {
        variant: _temporal_features(cache, metadata, variant=variant).float() for variant in VARIANTS
    }
...
def _labels(meta: pd.DataFrame) -> torch.Tensor:
    values = meta["stable_positive_v2_case"].astype(int).to_numpy(copy=True)
...
def _fit_head(
    ...
    *,
    ...
    model = PrototypeMemoryDeferHead(
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Prototype Temporal Precision vs Coverage")
    axes[1].set_title("Prototype Temporal Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    train_features, train_risk_sets = _feature_sets(train_cache, train_meta)
    ...
    for variant in VARIANTS:
        risk_train_norm, _ = _normalize(train_risk_sets[variant], train_risk_sets[variant])
        model = _fit_head(
            train_norm,
            risk_train_norm,
            target,
        ...
        for split_name, eval_meta, eval_features, eval_risk_sets in eval_payloads:
            _, eval_norm = _normalize(train_features, eval_features)
            _, risk_norm_eval = _normalize(train_risk_sets[variant], eval_risk_sets[variant])
            with torch.no_grad():
                scores = model(eval_norm, risk_norm_eval).numpy()
