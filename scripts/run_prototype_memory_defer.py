...
VARIANTS = ("prototype_memory", "prototype_hybrid", "prototype_committee")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_memory_defer",
...
def _labels(meta: pd.DataFrame, *, variant: str) -> torch.Tensor:
    if variant == "prototype_committee":
        values = meta["stable_positive_v2_committee_case"].astype(int).to_numpy(copy=True)
    else:
...
def _fit_head(
    ...
    *,
    ...
    use_risk = variant != "prototype_memory"
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Prototype-Memory Precision vs Coverage")
    axes[1].set_title("Prototype-Memory Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        target = _labels(train_meta, variant=variant)
        ...
        for split_name, eval_meta, eval_features, eval_risk in eval_payloads:
            ...
            with torch.no_grad():
                scores = model(eval_norm, risk_norm_eval if variant != "prototype_memory" else None).squeeze(-1).numpy()
