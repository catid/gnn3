...
VARIANTS = ("pair_scalar_hybrid", "pair_metric_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_pair_context_defer",
...
def _pair_context_features(cache: dict[str, torch.Tensor], metadata: pd.DataFrame, *, variant: str) -> torch.Tensor:
    ...
    score_1 = cache["base_selection_scores"][batch_index, top1].float()[:, None]
    score_2 = cache["base_selection_scores"][batch_index, top2].float()[:, None]
    cost_1 = cache["candidate_cost_to_go"][batch_index, top1].float()[:, None]
    cost_2 = cache["candidate_cost_to_go"][batch_index, top2].float()[:, None]
    on_time_1 = cache["candidate_on_time"][batch_index, top1].float()[:, None]
    on_time_2 = cache["candidate_on_time"][batch_index, top2].float()[:, None]
    slack_1 = cache["candidate_slack"][batch_index, top1].float()[:, None]
    slack_2 = cache["candidate_slack"][batch_index, top2].float()[:, None]
    ...
    if variant == "pair_scalar_hybrid":
        return torch.cat([base_risk, scalar], dim=1)

    first_features = cache["candidate_features"][batch_index, top1].float()
    second_features = cache["candidate_features"][batch_index, top2].float()
    delta = first_features - second_features
    delta_norm = torch.linalg.vector_norm(delta, dim=1, keepdim=True) / math.sqrt(float(delta.size(1)))
    cosine = F.cosine_similarity(first_features, second_features, dim=1).unsqueeze(1)
    feature_metrics = torch.cat([delta_norm, cosine], dim=1)
    return torch.cat([base_risk, scalar, feature_metrics], dim=1)
...
def _feature_sets(cache: dict[str, torch.Tensor], metadata: pd.DataFrame) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    ...
    return decision, {
        variant: _pair_context_features(cache, metadata, variant=variant).float() for variant in VARIANTS
    }
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Prototype Pair-Context Precision vs Coverage")
    axes[1].set_title("Prototype Pair-Context Hard Near-Tie Delta Regret")
