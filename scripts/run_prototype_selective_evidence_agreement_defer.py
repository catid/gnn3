...
from gnn3.models.prototype_defer import SelectiveEvidenceAgreementPrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_selective_evidence_agree", "prototype_selective_evidence_agree_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_selective_evidence_agreement_defer",
...
def _attach_teacher_bank(meta: pd.DataFrame, teacher_bank: pd.DataFrame) -> pd.DataFrame:
    merge_cols = [
        *KEY_COLS,
        ...
        "compute_breaks_baseline_success",
    ]
    merged = meta.merge(teacher_bank[merge_cols], on=KEY_COLS, how="left")

    def _coalesce(col: str) -> pd.Series:
        if col in merged.columns:
            return merged[col]
        left = merged[f"{col}_x"] if f"{col}_x" in merged.columns else None
        right = merged[f"{col}_y"] if f"{col}_y" in merged.columns else None
        ...
        if right is None:
            return left
        return right.combine_first(left)

    merged["stable_positive_v2_case"] = _coalesce("stable_positive_v2_case").fillna(False).astype(bool)
    merged["harmful_teacher_bank_case"] = _coalesce("harmful_teacher_bank_case").fillna(False).astype(bool)
    merged["best_safe_teacher_gain"] = _coalesce("best_safe_teacher_gain").fillna(0.0).astype(float)
    merged["best_safe_teacher_target_match"] = _coalesce("best_safe_teacher_target_match").fillna(False).astype(bool)
    merged["best_safe_teacher_delta_regret"] = _coalesce("best_safe_teacher_delta_regret").fillna(0.0).astype(float)
    merged["best_safe_teacher_delta_miss"] = _coalesce("best_safe_teacher_delta_miss").fillna(0.0).astype(float)
    merged["committee_support"] = _coalesce("committee_support").fillna(0).astype(int)
    merged["compute_recovers_baseline_error"] = _coalesce("compute_recovers_baseline_error").fillna(False).astype(bool)
    merged["compute_breaks_baseline_success"] = _coalesce("compute_breaks_baseline_success").fillna(False).astype(bool)
    return merged
...
def _weights(meta: pd.DataFrame, target: torch.Tensor) -> torch.Tensor:
    ...
    positive_mask = target.numpy().astype(bool)
...
def _fit_head(
    ...
    *,
    ...
    use_risk = variant == "prototype_selective_evidence_agree_hybrid"
    model = SelectiveEvidenceAgreementPrototypeDeferHead(
...
def _evaluate_budget(
    ...
    *,
    ...
    selected = top_fraction_mask(scores, budget_pct / 100.0)
    decision_frame = frame.copy()
    ...
    decision_frame["combined_target_match"] = np.where(
        selected,
        decision_frame["best_safe_teacher_target_match"],
        decision_frame["base_target_match"],
    )
    decision_frame["delta_regret"] = np.where(
        selected,
        decision_frame["best_safe_teacher_delta_regret"],
        0.0,
    )
    decision_frame["delta_miss"] = np.where(
        selected,
        decision_frame["best_safe_teacher_delta_miss"],
        0.0,
    )

    rows: list[dict[str, object]] = []
    for slice_name, mask in _slice_map(decision_frame).items():
        target = decision_frame.loc[mask].copy()
        if target.empty:
            continue
        selected_target = target["selected"].to_numpy(copy=True)
        selected_count = max(int(selected_target.sum()), 1)
        combined_target_match = target["combined_target_match"].to_numpy(copy=True)
        delta_regret = target["delta_regret"].to_numpy(copy=True)
        delta_miss = target["delta_miss"].to_numpy(copy=True)
        rows.append(
            {
                "variant": variant,
                "split": split,
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Selective-Evidence Precision vs Coverage")
    axes[1].set_title("Selective-Evidence Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        for split_name, eval_meta, eval_features, eval_risk in eval_payloads:
            ...
            with torch.no_grad():
                scores = model(eval_norm, risk_norm_eval if variant == "prototype_selective_evidence_agree_hybrid" else None)
                scores = scores.numpy()
            for budget_pct in args.coverage_budgets:
                summary_df, decisions_df = _evaluate_budget(
                    eval_meta,
                    scores,
                    budget_pct=budget_pct,
                    variant=variant,
        ...
        payload.append(
            {
                "variant": variant,
                "use_risk_branch": variant == "prototype_selective_evidence_agree_hybrid",
                "positive_prototypes": args.positive_prototypes,
                "negative_prototypes": args.negative_prototypes,
                "prototype_dim": args.prototype_dim,
                "hidden_dim": args.hidden_dim,
    ...
    _plot(summary, output_prefix.with_name(output_prefix.name + "_summary.png"))
