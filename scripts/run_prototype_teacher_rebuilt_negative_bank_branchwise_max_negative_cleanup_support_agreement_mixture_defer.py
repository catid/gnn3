...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_teacher_rebuilt_negative_bank_branchwise_max_negative_cleanup_support_agreement_mixture_defer",
...
def _rebuild_model(
    base_model: TeacherRebuiltNegativeBankBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead,
    ...
    *,
    ...
    with torch.no_grad():
        shared_negative_encoded = base_model.encode_shared(train_features[harmful_mask])
        dual_negative_encoded = base_model.encode_dual_negative(train_features[harmful_mask])
        shared_stable_encoded = base_model.encode_shared(train_features[stable_mask])
        dual_stable_encoded = base_model.encode_dual_negative(train_features[stable_mask])
        shared_bank, _ = base_model._shared_banks()
        _, dual_bank = base_model._dual_banks()
        harmful_weights = _teacher_candidate_weights(train_meta)
        rebuilt_shared, rebuilt_shared_support, shared_stats = _teacher_rebuild_bank(
            harmful_encoded=shared_negative_encoded,
            stable_encoded=shared_stable_encoded,
            harmful_weights=harmful_weights,
            learned_bank=shared_bank,
            bank_name="shared_negative",
            positive_overlap_penalty=args.positive_overlap_penalty,
        )
        rebuilt_dual, rebuilt_dual_support, dual_stats = _teacher_rebuild_bank(
            harmful_encoded=dual_negative_encoded,
            stable_encoded=dual_stable_encoded,
            harmful_weights=harmful_weights,
            learned_bank=dual_bank,
            bank_name="dual_negative",
            positive_overlap_penalty=args.positive_overlap_penalty,
        )

    rebuilt_model = TeacherRebuiltNegativeBankBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
    ...
    rebuilt_model.set_rebuilt_negative_banks(
        shared_negative_prototypes=rebuilt_shared,
        dual_negative_prototypes=rebuilt_dual,
        shared_negative_support=rebuilt_shared_support,
        dual_negative_support=rebuilt_dual_support,
    )

    summary_rows = [
        {"variant": variant, **shared_stats},
        {"variant": variant, **dual_stats},
    ...
    *,
    ...
    for variant in VARIANTS:
        base_model = _fit_head(
            ...
            train_weights,
            train_hard_negative,
            variant=variant.replace("prototype_teacher_rebuilt_negative_bank_", "prototype_"),
            epochs=args.epochs,
            lr=args.lr,
            prototype_dim=args.prototype_dim,
            positive_prototypes=args.positive_prototypes,
...
def _plot(summary: pd.DataFrame, output_prefix: Path) -> None:
    ...
    ax.set_title("Teacher-rebuilt negative-bank branchwise-max support agreement mixture")
