...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--margin-delta-scale", type=float, default=0.4)
    parser.add_argument("--margin-regularization-weight", type=float, default=0.01)
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_tail_margin_calibrated_branchwise_max_negative_cleanup_support_agreement_mixture_defer",
...
def _fit_head(
    ...
    *,
    ...
    margin_delta_scale: float,
    margin_regularization_weight: float,
    device: torch.device,
) -> TailMarginCalibratedBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead:
    use_risk = variant == "prototype_tail_margin_calibrated_branchwise_max_negative_cleanup_support_agree_mix_hybrid"
    model = TailMarginCalibratedBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
        ...
        margin_delta_scale=margin_delta_scale,
    ).to(device)
    ...
    for _ in range(epochs):
        ...
        margin_reg = model.margin_regularization()
        loss = (
            (per_row * w).mean()
            + 0.1 * reg
            + 0.01 * support_reg
            + 0.01 * tail_reg
            + margin_regularization_weight * margin_reg
        )
        loss.backward()
        optimizer.step()
    return model.cpu()
    ...
    *,
    ...
    for variant in VARIANTS:
        model = _fit_head(
            ...
            margin_delta_scale=args.margin_delta_scale,
            margin_regularization_weight=args.margin_regularization_weight,
            device=device,
        )
...
def _plot(summary: pd.DataFrame, output_prefix: Path) -> None:
    ...
    ax.set_title("Tail-margin-calibrated branchwise-max negative-cleanup support agreement mixture")
...
def main() -> None:
    ...
    summary, decisions, margin_stats = _fit_and_score(train_cache, train_meta, eval_caches, eval_meta, args=args)
    ...
    output = {
        ...
        "margin_summary": margin_stats.to_dict(orient="records"),
    }
