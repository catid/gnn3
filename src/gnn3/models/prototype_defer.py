...
class SoftTailSupportAgreementMixturePrototypeDeferHead(SupportWeightedAgreementMixturePrototypeDeferHead):
    """Support-weighted agreement mixture with soft suppression of low-ranked logits."""
    ...
    def __init__(
        ...
        *,
        ...
        self.tail_shrink_scale = tail_shrink_scale
    ...
    def _bounded_tail_strength(self, raw_strength: torch.Tensor) -> torch.Tensor:
        return self.tail_shrink_scale * torch.sigmoid(raw_strength)
    ...
    def _soft_tail_logits(self, logits: torch.Tensor, raw_strength: torch.Tensor) -> torch.Tensor:
        ...
        penalty = self._bounded_tail_strength(raw_strength) * torch.relu((lead - logits) - self.tail_margin)
    ...
    def _shared_score(self, shared_encoded: torch.Tensor) -> torch.Tensor:
        ...
        pos_logits = scale * shared_encoded @ pos.T + self._bounded_support(self.shared_positive_support)
        neg_logits = scale * shared_encoded @ neg.T + self._bounded_support(self.shared_negative_support)
        pos_logits = self._soft_tail_logits(pos_logits, self.shared_positive_tail)
        neg_logits = self._soft_tail_logits(neg_logits, self.shared_negative_tail)
        return torch.logsumexp(pos_logits, dim=1) - torch.logsumexp(neg_logits, dim=1)
    ...
    def _dual_score(self, positive_encoded: torch.Tensor, negative_encoded: torch.Tensor) -> torch.Tensor:
        ...
        pos_logits = scale * positive_encoded @ pos.T + self._bounded_support(self.dual_positive_support)
        neg_logits = scale * negative_encoded @ neg.T + self._bounded_support(self.dual_negative_support)
        pos_logits = self._soft_tail_logits(pos_logits, self.dual_positive_tail)
        neg_logits = self._soft_tail_logits(neg_logits, self.dual_negative_tail)
        return torch.logsumexp(pos_logits, dim=1) - torch.logsumexp(neg_logits, dim=1)

    def tail_regularization(self) -> torch.Tensor:
        strengths = torch.stack(
            [
                self._bounded_tail_strength(self.shared_positive_tail),
                self._bounded_tail_strength(self.shared_negative_tail),
                self._bounded_tail_strength(self.dual_positive_tail),
                self._bounded_tail_strength(self.dual_negative_tail),
            ]
        )
...
class NegativeTailSupportAgreementMixturePrototypeDeferHead(SupportWeightedAgreementMixturePrototypeDeferHead):
    """Support-weighted agreement mixture with soft tail suppression only on negative banks."""
...
class MassAwareSharpNegativeTailSupportAgreementMixturePrototypeDeferHead(
    NegativeTailSupportAgreementMixturePrototypeDeferHead
):
    """Negative-tail cleanup that strengthens when the full negative tail mass is diffuse."""
    ...
    def __init__(
        ...
        *,
        ...
        tail_margin: float = 0.5,
        tail_shrink_scale: float = 2.0,
        tail_mass_center: float = 0.5,
        tail_mass_scale: float = 4.0,
    ) -> None:
        super().__init__(
            feature_dim,
            risk_dim=risk_dim,
        ...
        self.tail_mass_center = tail_mass_center
        self.tail_mass_scale = tail_mass_scale
    ...
    def _negative_tail_gate(self, logits: torch.Tensor) -> torch.Tensor:
        ...
        lead_idx = logits.argmax(dim=1, keepdim=True)
        masked = logits.masked_fill(
            torch.zeros_like(logits, dtype=torch.bool).scatter(1, lead_idx, True),
            torch.finfo(logits.dtype).min,
        )
        lead = logits.gather(1, lead_idx).squeeze(1)
        tail_mass = torch.logsumexp(masked, dim=1)
        tail_gap = lead - tail_mass
        return torch.sigmoid(self.tail_mass_scale * (self.tail_mass_center - tail_gap)).unsqueeze(1)
    ...
    def _dual_score(self, positive_encoded: torch.Tensor, negative_encoded: torch.Tensor) -> torch.Tensor:
        ...
        pos_logits = scale * positive_encoded @ pos.T + self._bounded_support(self.dual_positive_support)
        neg_logits = scale * negative_encoded @ neg.T + self._bounded_support(self.dual_negative_support)
        neg_logits = self._adaptive_soft_tail_logits(neg_logits, self.dual_negative_tail)
        return torch.logsumexp(pos_logits, dim=1) - torch.logsumexp(neg_logits, dim=1)


class SharpMassNegativeTailSupportAgreementMixturePrototypeDeferHead(
    NegativeTailSupportAgreementMixturePrototypeDeferHead
):
    """Negative-tail cleanup that activates from either local sharpness or broader tail mass."""
    ...
    def _sharpness_gate(self, logits: torch.Tensor) -> torch.Tensor:
        if logits.size(1) < 2:
            return torch.ones(logits.size(0), 1, device=logits.device, dtype=logits.dtype)
        top2 = logits.topk(k=2, dim=1).values
        gap = top2[:, 0] - top2[:, 1]
    ...
    def _tail_mass_gate(self, logits: torch.Tensor) -> torch.Tensor:
        if logits.size(1) < 2:
            return torch.ones(logits.size(0), 1, device=logits.device, dtype=logits.dtype)
        lead_idx = logits.argmax(dim=1, keepdim=True)
        mask = torch.zeros_like(logits, dtype=torch.bool).scatter(1, lead_idx, True)
    ...
    def _negative_tail_gate(self, logits: torch.Tensor) -> torch.Tensor:
        sharp_gate = self._sharpness_gate(logits)
        mass_gate = self._tail_mass_gate(logits)
        return 1.0 - (1.0 - sharp_gate) * (1.0 - mass_gate)
...
class BranchStrengthNegativeCleanupLiftSupportAgreementMixturePrototypeDeferHead(
    BranchStrengthSharpNegativeTailSupportAgreementMixturePrototypeDeferHead
):
    """Use branch-strength sharp cleanup as the base and add only a gated fixed-cleanup lift."""
    ...
    def __init__(
        ...
        *,
        ...
        self.cleanup_lift_gate = torch.nn.Sequential(
            torch.nn.Linear(8, hidden_dim),
            torch.nn.GELU(),
            torch.nn.Linear(hidden_dim, 1),
        )
        self.cleanup_lift_bias = torch.nn.Parameter(torch.tensor(-1.0, dtype=torch.float32))

    def _negative_gap(self, logits: torch.Tensor) -> torch.Tensor:
        if logits.size(1) < 2:
            return torch.zeros(logits.size(0), device=logits.device, dtype=logits.dtype)
        top2 = logits.topk(k=2, dim=1).values
        return top2[:, 0] - top2[:, 1]

    def _shared_branch_scores(self, shared_encoded: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        scale = self.shared_logit_scale.exp().clamp(min=1.0, max=64.0)
        pos, neg = self._shared_banks()
        pos_logits = scale * shared_encoded @ pos.T + self._bounded_support(self.shared_positive_support)
        neg_logits = scale * shared_encoded @ neg.T + self._bounded_support(self.shared_negative_support)
        ...
        return fixed_score, sharp_score, self._negative_gap(neg_logits)
    ...
    def _cleanup_lift_features(
        self,
        sharp_logits: torch.Tensor,
        fixed_logits: torch.Tensor,
        shared_neg_gap: torch.Tensor,
        dual_neg_gap: torch.Tensor,
    ) -> torch.Tensor:
        lift_gap = fixed_logits - sharp_logits
        mean_gap = 0.5 * (shared_neg_gap + dual_neg_gap)
        return torch.stack(
            [
                sharp_logits,
                fixed_logits,
    ...
    def forward(self, features: torch.Tensor, risk_features: torch.Tensor | None = None) -> torch.Tensor:
        ...
        fixed_shared, sharp_shared, shared_neg_gap = self._shared_branch_scores(shared_encoded)
        fixed_dual, sharp_dual, dual_neg_gap = self._dual_branch_scores(dual_pos_encoded, dual_neg_encoded)

        fixed_logits = self._mixture_logit(fixed_shared, fixed_dual)
        sharp_logits = self._mixture_logit(sharp_shared, sharp_dual)
        lift_features = self._cleanup_lift_features(sharp_logits, fixed_logits, shared_neg_gap, dual_neg_gap)
        lift_gate = torch.sigmoid(self.cleanup_lift_gate(lift_features).squeeze(-1) + self.cleanup_lift_bias)
        lift = lift_gate * torch.relu(fixed_logits - sharp_logits)
        logits = sharp_logits + lift
        if self.risk_branch is not None and risk_features is not None:
            logits = logits + self.risk_branch(risk_features).squeeze(-1)
        return logits
...
class BranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
    BranchStrengthSharpNegativeTailSupportAgreementMixturePrototypeDeferHead
):
    """Take the fixed-vs-sharp max separately inside shared and dual branches before agreement mixing."""
...
class FixedRescueAnchoredDualLiftBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
    BranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead
):
    """Keep branchwise-max as the anchor and only lift through dual fixed-over-sharp rescue."""
    ...
    def __init__(
        ...
        *,
        ...
        self.fixed_rescue_lift_gate = torch.nn.Sequential(
            torch.nn.Linear(6, hidden_dim),
            torch.nn.GELU(),
            torch.nn.Linear(hidden_dim, 1),
        )
        self.fixed_rescue_lift_bias = torch.nn.Parameter(torch.tensor(-1.0, dtype=torch.float32))

    def _fixed_rescue_lift_features(
        self,
        branch_shared: torch.Tensor,
        branch_dual: torch.Tensor,
        fixed_shared: torch.Tensor,
        sharp_shared: torch.Tensor,
        fixed_dual: torch.Tensor,
        sharp_dual: torch.Tensor,
    ) -> torch.Tensor:
        dual_advantage = branch_dual - branch_shared
        dual_fixed_rescue = torch.relu(fixed_dual - sharp_dual)
        shared_fixed_rescue = torch.relu(fixed_shared - sharp_shared)
        return torch.stack(
            [
                branch_shared,
                branch_dual,
                ...
                dual_fixed_rescue,
                shared_fixed_rescue,
                (dual_fixed_rescue - shared_fixed_rescue).abs(),
            ],
            dim=1,
        )
    ...
    def forward(self, features: torch.Tensor, risk_features: torch.Tensor | None = None) -> torch.Tensor:
        ...
        dual_advantage = torch.relu(branch_dual - branch_shared)
        dual_fixed_rescue = torch.relu(fixed_dual - sharp_dual)
        rescue_budget = torch.minimum(dual_advantage, dual_fixed_rescue)
        lift_gate = torch.sigmoid(
            self.fixed_rescue_lift_gate(
                self._fixed_rescue_lift_features(
                    branch_shared,
                    branch_dual,
                    fixed_shared,
                    sharp_shared,
            ...
            + self.fixed_rescue_lift_bias
        )
        logits = logits + lift_gate * rescue_budget
        if self.risk_branch is not None and risk_features is not None:
            logits = logits + self.risk_branch(risk_features).squeeze(-1)
        return logits


class RescueWeightedAnchoredDualLiftBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
    AnchoredDualLiftBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead
):
    """Scale the anchored dual lift by structured dual fixed-rescue strength."""
    ...
    def forward(self, features: torch.Tensor, risk_features: torch.Tensor | None = None) -> torch.Tensor:
        ...
        rescue_strength = torch.sigmoid(
            self.rescue_weight_scale * (fixed_dual - sharp_dual) + self.rescue_weight_bias
        )
        logits = logits + lift_gate * rescue_strength * torch.relu(branch_dual - branch_shared)
        if self.risk_branch is not None and risk_features is not None:
            logits = logits + self.risk_branch(risk_features).squeeze(-1)
        return logits


class ConfidentRescueWeightedAnchoredDualLiftBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
    AnchoredDualLiftBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead
):
    """Apply rescue-weighted lift, but only strongly when the anchor score is already confident."""
    ...
    def __init__(
        ...
        *,
        ...
        self.rescue_weight_scale = torch.nn.Parameter(torch.tensor(1.0, dtype=torch.float32))
        self.rescue_weight_bias = torch.nn.Parameter(torch.tensor(0.0, dtype=torch.float32))
        self.confidence_gate_scale = torch.nn.Parameter(torch.tensor(1.0, dtype=torch.float32))
        self.confidence_gate_bias = torch.nn.Parameter(torch.tensor(0.0, dtype=torch.float32))
    ...
    def forward(self, features: torch.Tensor, risk_features: torch.Tensor | None = None) -> torch.Tensor:
        ...
        confidence_gate = torch.sigmoid(self.confidence_gate_scale * logits + self.confidence_gate_bias)
        logits = logits + lift_gate * rescue_weight * confidence_gate * torch.relu(branch_dual - branch_shared)
        if self.risk_branch is not None and risk_features is not None:
            logits = logits + self.risk_branch(risk_features).squeeze(-1)
        return logits
...
class HardNegativeConditionedBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
    BranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead
):
    """Branchwise max with externally selected negative-bank masks."""
    ...
    def __init__(
        ...
        *,
        ...
        self.register_buffer("shared_negative_keep_mask", torch.ones(negative_prototypes, dtype=torch.bool))
        self.register_buffer("dual_negative_keep_mask", torch.ones(negative_prototypes, dtype=torch.bool))

    def set_negative_keep_masks(self, *, shared_keep_mask: torch.Tensor, dual_keep_mask: torch.Tensor) -> None:
        shared_keep = shared_keep_mask.to(dtype=torch.bool, device=self.shared_negative_keep_mask.device)
        dual_keep = dual_keep_mask.to(dtype=torch.bool, device=self.dual_negative_keep_mask.device)
        if shared_keep.shape != self.shared_negative_keep_mask.shape:
            raise ValueError("shared_keep_mask has wrong shape.")
        if dual_keep.shape != self.dual_negative_keep_mask.shape:
            raise ValueError("dual_keep_mask has wrong shape.")
        if not bool(shared_keep.any()):
            raise ValueError("shared_keep_mask must keep at least one prototype.")
        if not bool(dual_keep.any()):
            raise ValueError("dual_keep_mask must keep at least one prototype.")
        self.shared_negative_keep_mask.copy_(shared_keep)
        self.dual_negative_keep_mask.copy_(dual_keep)
    ...
    def _shared_branch_scores(self, shared_encoded: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        pos_logits = scale * shared_encoded @ pos.T + self._bounded_support(self.shared_positive_support)
        neg_logits = scale * shared_encoded @ neg.T + self._bounded_support(self.shared_negative_support)
        neg_logits = self._apply_hard_bank_mask(neg_logits, self.shared_negative_keep_mask)
        fixed_score = torch.logsumexp(pos_logits, dim=1) - torch.logsumexp(
            self._soft_tail_logits(neg_logits, self.shared_negative_tail),
            dim=1,
        )
    ...
    def _dual_branch_scores(
        ...
        pos_logits = scale * positive_encoded @ pos.T + self._bounded_support(self.dual_positive_support)
        neg_logits = scale * negative_encoded @ neg.T + self._bounded_support(self.dual_negative_support)
        neg_logits = self._apply_hard_bank_mask(neg_logits, self.dual_negative_keep_mask)
        fixed_score = torch.logsumexp(pos_logits, dim=1) - torch.logsumexp(
            self._soft_tail_logits(neg_logits, self.dual_negative_tail),
            dim=1,
        )
    ...
    def keep_summary(self) -> dict[str, float]:
        return {
            "shared_negative_kept": float(self.shared_negative_keep_mask.sum().item()),
            "shared_negative_total": float(self.shared_negative_keep_mask.numel()),
            "dual_negative_kept": float(self.dual_negative_keep_mask.sum().item()),
            "dual_negative_total": float(self.dual_negative_keep_mask.numel()),
        }
...
class DualTeacherRebuiltNegativeBankBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
    BranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead
):
    """Branchwise max with only the dual negative bank rebuilt from teacher-marked harmful states."""
...
class DualTeacherRebuiltNegativeBankRescueWeightedAnchoredDualLiftBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
    RescueWeightedAnchoredDualLiftBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead
):
    """Rescue-weighted anchored branchwise max with only the dual negative bank rebuilt."""
...
class MaxDualTeacherRebuiltNegativeBankRescueWeightedAnchoredDualLiftBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
    RescueWeightedAnchoredDualLiftBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead
):
    """Take a hard max between learned and rebuilt dual negative-bank score families."""
    ...
    def _dual_branch_scores(
        ...
        fixed_score = torch.maximum(learned_fixed, rebuilt_fixed)
        sharp_score = torch.maximum(learned_sharp, rebuilt_sharp)
        return fixed_score, sharp_score
...
class TailMarginCalibratedBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
    ...
    """Branchwise max with branch/path-specific bounded tail-margin calibration."""
    ...
    def __init__(
        ...
        *,
        ...
        self.shared_fixed_margin_delta = torch.nn.Parameter(torch.zeros((), dtype=torch.float32))
        self.shared_sharp_margin_delta = torch.nn.Parameter(torch.zeros((), dtype=torch.float32))
        self.dual_fixed_margin_delta = torch.nn.Parameter(torch.zeros((), dtype=torch.float32))
        self.dual_sharp_margin_delta = torch.nn.Parameter(torch.zeros((), dtype=torch.float32))

    def _bounded_margin(self, raw_delta: torch.Tensor) -> torch.Tensor:
        return self.tail_margin + self.margin_delta_scale * torch.tanh(raw_delta)

    def _soft_tail_logits_with_margin(
        self,
        logits: torch.Tensor,
        raw_strength: torch.Tensor,
        *,
        ...
        lead = logits.max(dim=1, keepdim=True).values
        penalty = self._bounded_tail_strength(raw_strength) * torch.relu((lead - logits) - margin)
        return logits - penalty

    def _adaptive_soft_tail_logits_with_margin(
        self,
        logits: torch.Tensor,
        raw_strength: torch.Tensor,
        *,
        ...
        lead = logits.max(dim=1, keepdim=True).values
        gate = self._negative_tail_gate(logits)
        penalty = gate * self._bounded_branch_tail_strength(raw_strength, raw_scale) * torch.relu((lead - logits) - margin)
        return logits - penalty
    ...
    def _shared_branch_scores(self, shared_encoded: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        neg_logits = scale * shared_encoded @ neg.T + self._bounded_support(self.shared_negative_support)
        fixed_score = torch.logsumexp(pos_logits, dim=1) - torch.logsumexp(
            self._soft_tail_logits_with_margin(
                neg_logits,
                self.shared_negative_tail,
                margin=self._bounded_margin(self.shared_fixed_margin_delta),
            ),
            dim=1,
        )
        sharp_score = torch.logsumexp(pos_logits, dim=1) - torch.logsumexp(
            self._adaptive_soft_tail_logits_with_margin(
                neg_logits,
                self.shared_negative_tail,
                raw_scale=self.shared_tail_scale,
                margin=self._bounded_margin(self.shared_sharp_margin_delta),
            ),
            dim=1,
        )
        return fixed_score, sharp_score
    ...
    def _dual_branch_scores(
        ...
        fixed_score = torch.logsumexp(pos_logits, dim=1) - torch.logsumexp(
            self._soft_tail_logits_with_margin(
                neg_logits,
                self.dual_negative_tail,
                margin=self._bounded_margin(self.dual_fixed_margin_delta),
            ),
            dim=1,
        )
        sharp_score = torch.logsumexp(pos_logits, dim=1) - torch.logsumexp(
            self._adaptive_soft_tail_logits_with_margin(
                ...
                self.dual_negative_tail,
                raw_scale=self.dual_tail_scale,
                margin=self._bounded_margin(self.dual_sharp_margin_delta),
            ),
            dim=1,
        )
        return fixed_score, sharp_score

    def margin_regularization(self) -> torch.Tensor:
        deltas = torch.stack(
            [
                self.shared_fixed_margin_delta,
                self.shared_sharp_margin_delta,
                self.dual_fixed_margin_delta,
                self.dual_sharp_margin_delta,
            ]
        )
        return deltas.pow(2).mean()
    ...
    def margin_summary(self) -> dict[str, float]:
        return {
            "shared_fixed_margin": float(self._bounded_margin(self.shared_fixed_margin_delta).item()),
            "shared_sharp_margin": float(self._bounded_margin(self.shared_sharp_margin_delta).item()),
            "dual_fixed_margin": float(self._bounded_margin(self.dual_fixed_margin_delta).item()),
            "dual_sharp_margin": float(self._bounded_margin(self.dual_sharp_margin_delta).item()),
        }


class FixedTailMarginBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
    BranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead
):
    """Branchwise max with explicit fixed shared/dual fixed/sharp cleanup margins."""
    ...
    def __init__(
        ...
        *,
        ...
        sharpness_center: float = 0.75,
        sharpness_scale: float = 4.0,
        shared_fixed_margin: float = 0.5,
        shared_sharp_margin: float = 0.5,
        dual_fixed_margin: float = 0.5,
        dual_sharp_margin: float = 0.5,
    ) -> None:
        super().__init__(
            feature_dim,
            risk_dim=risk_dim,
        ...
        self.shared_fixed_margin = float(shared_fixed_margin)
        self.shared_sharp_margin = float(shared_sharp_margin)
        self.dual_fixed_margin = float(dual_fixed_margin)
        self.dual_sharp_margin = float(dual_sharp_margin)

    def set_margins(
        self,
        *,
        shared_fixed_margin: float,
        shared_sharp_margin: float,
        dual_fixed_margin: float,
        dual_sharp_margin: float,
    ) -> None:
        self.shared_fixed_margin = float(shared_fixed_margin)
        self.shared_sharp_margin = float(shared_sharp_margin)
        self.dual_fixed_margin = float(dual_fixed_margin)
    ...
    def _shared_branch_scores(self, shared_encoded: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        fixed_score = torch.logsumexp(pos_logits, dim=1) - torch.logsumexp(
            self._soft_tail_logits_with_margin(
                neg_logits,
                self.shared_negative_tail,
                margin=self.shared_fixed_margin,
            ),
            dim=1,
        )
        sharp_score = torch.logsumexp(pos_logits, dim=1) - torch.logsumexp(
            self._adaptive_soft_tail_logits_with_margin(
                ...
                self.shared_negative_tail,
                raw_scale=self.shared_tail_scale,
                margin=self.shared_sharp_margin,
            ),
            dim=1,
        )
        return fixed_score, sharp_score
    ...
    def margin_summary(self) -> dict[str, float]:
        return {
            "shared_fixed_margin": self.shared_fixed_margin,
            "shared_sharp_margin": self.shared_sharp_margin,
            "dual_fixed_margin": self.dual_fixed_margin,
            "dual_sharp_margin": self.dual_sharp_margin,
        }
...
class BranchwiseMarginMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
    BranchStrengthSharpNegativeTailSupportAgreementMixturePrototypeDeferHead
):
    """Take a branchwise max only after fixed cleanup exceeds sharp cleanup by a learned branch margin."""
    ...
    def __init__(
        ...
        *,
        ...
        sharpness_center: float = 0.75,
        sharpness_scale: float = 4.0,
        shared_branch_margin_init: float = 0.2,
        dual_branch_margin_init: float = 0.2,
    ) -> None:
        super().__init__(
            feature_dim,
            risk_dim=risk_dim,
        ...
        self.shared_branch_margin = torch.nn.Parameter(torch.tensor(shared_branch_margin_init, dtype=torch.float32))
        self.dual_branch_margin = torch.nn.Parameter(torch.tensor(dual_branch_margin_init, dtype=torch.float32))
    ...
    def _bounded_branch_margin(self, raw_margin: torch.Tensor) -> torch.Tensor:
        return F.softplus(raw_margin).clamp(min=0.0, max=2.0)
    ...
    def forward(self, features: torch.Tensor, risk_features: torch.Tensor | None = None) -> torch.Tensor:
        ...
        fixed_shared, sharp_shared = self._shared_branch_scores(shared_encoded)
        fixed_dual, sharp_dual = self._dual_branch_scores(dual_pos_encoded, dual_neg_encoded)
        branch_shared = sharp_shared + torch.relu(
            fixed_shared - sharp_shared - self._bounded_branch_margin(self.shared_branch_margin)
        )
        branch_dual = sharp_dual + torch.relu(
            fixed_dual - sharp_dual - self._bounded_branch_margin(self.dual_branch_margin)
        )
        logits = self._mixture_logit(branch_shared, branch_dual)
        if self.risk_branch is not None and risk_features is not None:
            logits = logits + self.risk_branch(risk_features).squeeze(-1)
...
class JointSupportBranchwiseNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
    BranchStrengthSharpNegativeTailSupportAgreementMixturePrototypeDeferHead
):
    """Add only the fixed-cleanup gain jointly supported by both shared and dual branches."""
    ...
    def forward(self, features: torch.Tensor, risk_features: torch.Tensor | None = None) -> torch.Tensor:
        ...
        fixed_shared, sharp_shared = self._shared_branch_scores(shared_encoded)
        fixed_dual, sharp_dual = self._dual_branch_scores(dual_pos_encoded, dual_neg_encoded)
        shared_gain = torch.relu(fixed_shared - sharp_shared)
        dual_gain = torch.relu(fixed_dual - sharp_dual)
        common_gain = torch.minimum(shared_gain, dual_gain)
        branch_shared = sharp_shared + common_gain
        branch_dual = sharp_dual + common_gain
        logits = self._mixture_logit(branch_shared, branch_dual)
        if self.risk_branch is not None and risk_features is not None:
            logits = logits + self.risk_branch(risk_features).squeeze(-1)
        return logits
...
class LearnedGateNegativeTailSupportAgreementMixturePrototypeDeferHead(
    ...
    """Negative-tail cleanup with learned shared and dual gates from internal bank summaries."""
    ...
    def __init__(
        ...
        *,
        ...
        self.shared_gate = torch.nn.Linear(3, 1)
        self.dual_gate = torch.nn.Linear(3, 1)

    def _negative_tail_features(self, logits: torch.Tensor) -> torch.Tensor:
        lead = logits.max(dim=1).values
        if logits.size(1) < 2:
            zeros = torch.zeros_like(lead)
            return torch.stack([lead, zeros, zeros], dim=1)
        top2 = logits.topk(k=2, dim=1).values
        ...
        tail_mass = torch.logsumexp(masked, dim=1)
        tail_gap = lead - tail_mass
        return torch.stack([lead, top_gap, tail_gap], dim=1)
    ...
    def _adaptive_soft_tail_logits(
        ...
        raw_strength: torch.Tensor,
        *,
        gate_layer: torch.nn.Linear,
    ) -> torch.Tensor:
        lead = logits.max(dim=1, keepdim=True).values
        gate = torch.sigmoid(gate_layer(self._negative_tail_features(logits)))
        penalty = gate * self._bounded_tail_strength(raw_strength) * torch.relu((lead - logits) - self.tail_margin)
        return logits - penalty
...
class NegativeCleanupBlendSupportAgreementMixturePrototypeDeferHead(NegativeTailSupportAgreementMixturePrototypeDeferHead):
    """Blend fixed and sharp negative-tail cleanup paths per state."""
    ...
    def __init__(
        ...
        *,
        ...
        self.sharpness_center = sharpness_center
        self.sharpness_scale = sharpness_scale
        self.cleanup_blend_gate = torch.nn.Sequential(
            torch.nn.Linear(8, hidden_dim),
            torch.nn.GELU(),
            torch.nn.Linear(hidden_dim, 1),
        )
        self.cleanup_blend_bias = torch.nn.Parameter(torch.tensor(-0.5, dtype=torch.float32))
    ...
    def _shared_branch_scores(self, shared_encoded: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        sharp_score = torch.logsumexp(pos_logits, dim=1) - torch.logsumexp(
            self._adaptive_soft_tail_logits(neg_logits, self.shared_negative_tail),
            dim=1,
        )
        return fixed_score, sharp_score, self._negative_gap(neg_logits)
    ...
    def _dual_branch_scores(
        ...
        sharp_score = torch.logsumexp(pos_logits, dim=1) - torch.logsumexp(
            self._adaptive_soft_tail_logits(neg_logits, self.dual_negative_tail),
            dim=1,
        )
        return fixed_score, sharp_score, self._negative_gap(neg_logits)
    ...
    def _cleanup_blend_features(
        ...
        diff = sharp_logits - fixed_logits
        mean_gap = 0.5 * (shared_neg_gap + dual_neg_gap)
        return torch.stack(
            [
                fixed_logits,
                sharp_logits,
                diff.abs(),
                fixed_logits * sharp_logits,
                shared_neg_gap,
                dual_neg_gap,
                mean_gap,
                (shared_neg_gap - dual_neg_gap).abs(),
            ],
            dim=1,
        )
    ...
    def forward(self, features: torch.Tensor, risk_features: torch.Tensor | None = None) -> torch.Tensor:
        ...
        fixed_logits = self._mixture_logit(fixed_shared, fixed_dual)
        sharp_logits = self._mixture_logit(sharp_shared, sharp_dual)
        blend_features = self._cleanup_blend_features(fixed_logits, sharp_logits, shared_neg_gap, dual_neg_gap)
        blend = torch.sigmoid(self.cleanup_blend_gate(blend_features).squeeze(-1) + self.cleanup_blend_bias)
        logits = fixed_logits + blend * (sharp_logits - fixed_logits)
        if self.risk_branch is not None and risk_features is not None:
            logits = logits + self.risk_branch(risk_features).squeeze(-1)
        return logits


class NegativeCleanupMaxSupportAgreementMixturePrototypeDeferHead(NegativeTailSupportAgreementMixturePrototypeDeferHead):
    """Take a max-style lift from fixed negative cleanup over sharp negative cleanup."""
    ...
    def _dual_branch_scores(self, positive_encoded: torch.Tensor, negative_encoded: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        scale = self.dual_logit_scale.exp().clamp(min=1.0, max=64.0)
        pos, neg = self._dual_banks()
        pos_logits = scale * positive_encoded @ pos.T + self._bounded_support(self.dual_positive_support)
        neg_logits = scale * negative_encoded @ neg.T + self._bounded_support(self.dual_negative_support)
...
class NegativeCleanupLiftSupportAgreementMixturePrototypeDeferHead(NegativeTailSupportAgreementMixturePrototypeDeferHead):
    """Keep sharp negative cleanup as the base and add only a gated positive fixed-branch lift."""
...
class DualSharpNegativeTailSupportAgreementMixturePrototypeDeferHead(NegativeTailSupportAgreementMixturePrototypeDeferHead):
    """Sharpness-gated negative cleanup only on the dual branch, fixed cleanup on the shared branch."""
...
class FloorSharpNegativeTailSupportAgreementMixturePrototypeDeferHead(NegativeTailSupportAgreementMixturePrototypeDeferHead):
    ...
    def __init__(
        ...
        *,
        ...
        support_scale: float = 2.0,
        tail_margin: float = 0.5,
        tail_shrink_scale: float = 1.0,
        adaptive_tail_shrink_scale: float = 1.5,
        sharpness_center: float = 0.75,
        sharpness_scale: float = 4.0,
    ) -> None:
        super().__init__(
        ...
        self.adaptive_tail_shrink_scale = adaptive_tail_shrink_scale
        self.sharpness_center = sharpness_center
        self.sharpness_scale = sharpness_scale
        self.shared_negative_adaptive_tail = torch.nn.Parameter(torch.zeros((), dtype=torch.float32))
        self.dual_negative_adaptive_tail = torch.nn.Parameter(torch.zeros((), dtype=torch.float32))

    def _bounded_adaptive_tail_strength(self, raw_strength: torch.Tensor) -> torch.Tensor:
        return self.adaptive_tail_shrink_scale * torch.sigmoid(raw_strength)
    ...
    def _floor_plus_adaptive_soft_tail_logits(
        self,
        logits: torch.Tensor,
        raw_floor_strength: torch.Tensor,
        raw_adaptive_strength: torch.Tensor,
    ) -> torch.Tensor:
        lead = logits.max(dim=1, keepdim=True).values
        tail_gap = torch.relu((lead - logits) - self.tail_margin)
        floor_penalty = self._bounded_tail_strength(raw_floor_strength) * tail_gap
        adaptive_penalty = self._negative_tail_gate(logits) * self._bounded_adaptive_tail_strength(raw_adaptive_strength) * tail_gap
        return logits - floor_penalty - adaptive_penalty
    ...
    def _shared_score(self, shared_encoded: torch.Tensor) -> torch.Tensor:
        ...
        pos_logits = scale * shared_encoded @ pos.T + self._bounded_support(self.shared_positive_support)
        neg_logits = scale * shared_encoded @ neg.T + self._bounded_support(self.shared_negative_support)
        neg_logits = self._floor_plus_adaptive_soft_tail_logits(
            neg_logits,
            self.shared_negative_tail,
            self.shared_negative_adaptive_tail,
        )
        return torch.logsumexp(pos_logits, dim=1) - torch.logsumexp(neg_logits, dim=1)
    ...
    def _dual_score(self, positive_encoded: torch.Tensor, negative_encoded: torch.Tensor) -> torch.Tensor:
        ...
        neg_logits = self._floor_plus_adaptive_soft_tail_logits(
            neg_logits,
            self.dual_negative_tail,
            self.dual_negative_adaptive_tail,
        )
        return torch.logsumexp(pos_logits, dim=1) - torch.logsumexp(neg_logits, dim=1)
    ...
    def tail_regularization(self) -> torch.Tensor:
        strengths = torch.stack(
            [
                self._bounded_tail_strength(self.shared_negative_tail),
                self._bounded_tail_strength(self.dual_negative_tail),
                self._bounded_adaptive_tail_strength(self.shared_negative_adaptive_tail),
                self._bounded_adaptive_tail_strength(self.dual_negative_adaptive_tail),
            ]
        )
        return strengths.mean()
...
class SupportWeightedEvidenceAgreementPrototypeDeferHead(EvidenceAgreementPrototypeDeferHead):
    """Evidence-agreement head with bounded per-prototype support weights."""
