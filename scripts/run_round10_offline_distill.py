...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/round10_offline_distill",
...
def _teacher_actions(cache: dict[str, torch.Tensor], metadata: pd.DataFrame) -> torch.Tensor:
    if "teacher_next_hop" in metadata.columns:
        return torch.as_tensor(metadata["teacher_next_hop"].to_numpy(copy=True), dtype=torch.long)
    masked = _masked_scores(cache["compute_selection_scores"].float(), cache["candidate_mask"].bool())
...
def _train_gate(
    train_decision_features: torch.Tensor,
    helpful_mask: torch.Tensor,
    *,
    epochs: int,
    lr: float,
    hidden_dim: int,
    ...
    train_x = train_decision_features.float()
    ...
    x = norm_x.to(device)
    y = helpful_mask.float().to(device)
...
def _fit_student(
    ...
    *,
    ...
    teacher_action = _teacher_actions(train_cache, train_meta).to(device)
    base_action = base_scores.masked_fill(~mask, -1e9).argmax(dim=1)
    helpful_weight = torch.as_tensor(
        (1.0 + (-train_meta["delta_regret"].clip(upper=0.0)).to_numpy(copy=True)).astype(float),
        dtype=torch.float32,
        device=device,
    )
...
def _decision_metrics(predicted: torch.Tensor, cache: dict[str, torch.Tensor], metadata: pd.DataFrame) -> pd.DataFrame:
    ...
    base_pred = torch.as_tensor(metadata["base_predicted_next_hop"].to_numpy(copy=True), dtype=torch.long)
    target = cache["target_next_hop"].long()
    costs = cache["candidate_cost_to_go"].float()
    on_time = cache["candidate_on_time"].float()
    rows = metadata.copy()
    rows["predicted_next_hop_student"] = predicted.numpy()
    rows["student_target_match"] = (predicted == target).numpy()
    rows["base_target_match"] = metadata["base_target_match"].astype(bool)
    rows["disagreement"] = (predicted != base_pred).numpy()
    batch_index = torch.arange(predicted.size(0))
    rows["student_predicted_continuation_gap"] = costs[batch_index, predicted].numpy() - rows["best_candidate_cost"].to_numpy(copy=True)
    rows["student_predicted_on_time"] = (on_time[batch_index, predicted] > 0.5).numpy()
    rows["delta_regret_student"] = rows["student_predicted_continuation_gap"] - rows["base_predicted_continuation_gap"]
    rows["delta_miss_student"] = (~rows["student_predicted_on_time"]).astype(int) - (~rows["base_predicted_on_time"]).astype(int)
    ...
    for suite, suite_frame in rows.groupby("suite", sort=False):
        for slice_name, mask in slices.items():
            target_frame = suite_frame.loc[mask.loc[suite_frame.index]]
            summary_rows.append(
                {
                    "suite": suite,
                    "slice": slice_name,
                    "decisions": len(target_frame),
                    "disagreement": float(target_frame["disagreement"].mean()) if len(target_frame) else 0.0,
                    "correction_rate": float(target_frame["correction"].mean()) if len(target_frame) else 0.0,
                    "new_error_rate": float(target_frame["new_error"].mean()) if len(target_frame) else 0.0,
                    "base_target_match": float(target_frame["base_target_match"].mean()) if len(target_frame) else 0.0,
                    "student_target_match": float(target_frame["student_target_match"].mean()) if len(target_frame) else 0.0,
                    "mean_delta_regret": float(target_frame["delta_regret_student"].mean()) if len(target_frame) else 0.0,
                    "mean_delta_miss": float(target_frame["delta_miss_student"].mean()) if len(target_frame) else 0.0,
                }
            )
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    for split, group in hard.groupby("split", sort=False):
        axes[0].plot(group["variant"], group["student_target_match"], marker="o", label=split)
        axes[1].plot(group["variant"], group["mean_delta_regret"], marker="o", label=split)
...
def main() -> None:
    ...
    checkpoint_dir = output_prefix.with_name(output_prefix.name + "_checkpoints")
    ...
    for variant in VARIANTS:
        ...
        torch.save(payload, checkpoint_dir / f"{variant}.pt")
        for cache_path, meta_path in zip(args.eval_caches, args.eval_metadata, strict=True):
            ...
            summary_df = _decision_metrics(predicted, cache, meta)
            split = Path(cache_path).stem
