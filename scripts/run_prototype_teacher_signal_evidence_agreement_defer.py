...
from gnn3.models.prototype_defer import TeacherSignalEvidenceAgreementPrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_teacher_signal_evidence", "prototype_teacher_signal_evidence_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_teacher_signal_evidence_agreement_defer",
...
def _aux_targets(meta: pd.DataFrame) -> tuple[torch.Tensor, torch.Tensor]:
    committee = torch.as_tensor((meta["committee_support"].clip(lower=0, upper=4) / 4.0).to_numpy(copy=True), dtype=torch.float32)
    gain = torch.as_tensor((meta["best_safe_teacher_gain"].clip(lower=0.0, upper=1.5) / 1.5).to_numpy(copy=True), dtype=torch.float32)
    return committee, gain
...
def _fit_head(
    ...
    *,
    ...
    use_risk = variant == "prototype_teacher_signal_evidence_hybrid"
    model = TeacherSignalEvidenceAgreementPrototypeDeferHead(
    ...
    committee_y = committee_target.to(device)
    ...
    for _ in range(epochs):
        ...
        logits, committee_logits, gain_logits = model.forward_with_aux(x, risk if use_risk else None)
        per_row = F.binary_cross_entropy_with_logits(logits, y, reduction="none", pos_weight=pos_weight)
        committee_loss = F.binary_cross_entropy_with_logits(committee_logits, committee_y)
        gain_loss = F.binary_cross_entropy_with_logits(gain_logits, gain_y)
        reg = model.regularization(x, positive_mask=positive, hard_negative_mask=hard_negative)
        loss = (per_row * w).mean() + aux_weight * (committee_loss + gain_loss) + 0.1 * reg
        loss.backward()
        optimizer.step()
    return model.cpu()
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Teacher-Signal Evidence Precision vs Coverage")
    axes[1].set_title("Teacher-Signal Evidence Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        committee_target, gain_target = _aux_targets(train_meta)
        ...
        model = _fit_head(
            ...
            committee_target=committee_target,
        ...
        for split_name, eval_meta, eval_features, eval_risk in eval_payloads:
            ...
            with torch.no_grad():
                scores = model(
                    eval_norm,
                    risk_norm_eval if variant == "prototype_teacher_signal_evidence_hybrid" else None,
                ).numpy()
        ...
        payload.append(
            {
                "variant": variant,
                "use_risk_branch": variant == "prototype_teacher_signal_evidence_hybrid",
                "positive_prototypes": args.positive_prototypes,
                "negative_prototypes": args.negative_prototypes,
                "prototype_dim": args.prototype_dim,
                "hidden_dim": args.hidden_dim,
