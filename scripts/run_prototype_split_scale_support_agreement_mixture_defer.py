...
from gnn3.models.prototype_defer import SplitScaleSupportAgreementMixturePrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_split_scale_support_agree_mix", "prototype_split_scale_support_agree_mix_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_split_scale_support_agreement_mixture_defer",
...
def _fit_head(
    ...
    *,
    ...
    use_risk = variant == "prototype_split_scale_support_agree_mix_hybrid"
    model = SplitScaleSupportAgreementMixturePrototypeDeferHead(
    ...
    for _ in range(epochs):
        ...
        scale_reg = model.scale_regularization()
        loss = (per_row * w).mean() + 0.1 * reg + 0.01 * support_reg + 0.01 * scale_reg
        loss.backward()
        optimizer.step()
    return model.cpu()
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Split-Scale Support Agreement Precision vs Coverage")
    axes[1].set_title("Split-Scale Support Agreement Hard Near-Tie Delta Regret")
    ...
    for variant in VARIANTS:
        ...
        for split, eval_meta, eval_features, eval_risk in eval_sets:
            ...
            with torch.no_grad():
                scores = model(
                    eval_norm,
                    risk_norm_eval if variant == "prototype_split_scale_support_agree_mix_hybrid" else None,
                ).squeeze(-1).numpy()
