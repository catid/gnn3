...
from gnn3.models.prototype_defer import BandpassPrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_band_hybrid", "prototype_band_sharp_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_band_defer",
...
def _variant_params(variant: str) -> tuple[float, float]:
    if variant == "prototype_band_sharp_hybrid":
        return 0.75, 4.0
    return 1.0, 2.0
...
def _fit_head(
    ...
    *,
    ...
    band_width, band_sharpness = _variant_params(variant)
    model = BandpassPrototypeDeferHead(
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Prototype-Band Precision vs Coverage")
    axes[1].set_title("Prototype-Band Hard Near-Tie Delta Regret")
