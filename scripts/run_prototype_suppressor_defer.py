...
from gnn3.models.prototype_defer import SuppressorPrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_suppressor", "prototype_suppressor_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--neutral-prototypes", type=int, default=8)
    parser.add_argument("--harmful-prototypes", type=int, default=8)
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_suppressor_defer",
...
def _fit_head(
    ...
    *,
    ...
    use_risk = variant != "prototype_suppressor"
    model = SuppressorPrototypeDeferHead(
    ...
    y = target.to(device)
    w = weights.to(device)
    harmful_negative = harmful_negative_mask.to(device)
    neutral_negative = neutral_negative_mask.to(device)
    positive = y > 0.5
    ...
    for _ in range(epochs):
        ...
        reg = model.regularization(
            ...
            neutral_negative_mask=neutral_negative,
            harmful_negative_mask=harmful_negative,
        )
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Prototype-Suppressor Precision vs Coverage")
    axes[1].set_title("Prototype-Suppressor Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        harmful_negative = torch.as_tensor(train_meta["harmful_teacher_bank_case"].to_numpy(copy=True), dtype=torch.bool)
        neutral_negative = torch.as_tensor(
            ((~train_meta["stable_positive_v2_case"]) & (~train_meta["harmful_teacher_bank_case"])).to_numpy(copy=True),
            dtype=torch.bool,
        )
        model = _fit_head(
            train_norm,
        ...
        for split_name, eval_meta, eval_features, eval_risk in eval_payloads:
            ...
            with torch.no_grad():
                scores = model(eval_norm, risk_norm_eval if variant != "prototype_suppressor" else None).squeeze(-1).numpy()
