...
from gnn3.models.prototype_defer import LiftPrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_lift_hybrid", "prototype_lift_gated")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_lift_defer",
...
def _fit_head(
    ...
    *,
    ...
    model = LiftPrototypeDeferHead(
        ...
        gated=variant == "prototype_lift_gated",
    ).to(device)
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Prototype-Lift Precision vs Coverage")
    axes[1].set_title("Prototype-Lift Hard Near-Tie Delta Regret")
