...
from gnn3.models.prototype_defer import RiskConditionedSupportAgreementMixturePrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_risk_support_agree_mix", "prototype_risk_support_agree_mix_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_risk_conditioned_support_agreement_mixture_defer",
...
def _fit_head(
    ...
    *,
    ...
    use_risk = variant == "prototype_risk_support_agree_mix_hybrid"
    model = RiskConditionedSupportAgreementMixturePrototypeDeferHead(
    ...
    for _ in range(epochs):
        ...
        dynamic_support_reg = model.dynamic_support_regularization(risk)
        loss = (per_row * w).mean() + 0.1 * reg + 0.01 * support_reg + 0.01 * dynamic_support_reg
        loss.backward()
        optimizer.step()
    return model.cpu()
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Risk-Conditioned Support Agreement Precision vs Coverage")
    axes[1].set_title("Risk-Conditioned Support Agreement Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    for cache_path, meta_path in zip(args.eval_caches, args.eval_metadata, strict=True):
        eval_cache, eval_meta = _load(cache_path, meta_path)
        eval_meta = _attach_teacher_bank(eval_meta, teacher_bank)
        eval_features, eval_risk = _feature_sets(eval_cache, eval_meta)
        eval_sets.append((Path(cache_path).stem, eval_meta, eval_features, eval_risk))
    ...
    for variant in VARIANTS:
        ...
        for split, eval_meta, eval_features, eval_risk in eval_sets:
            _, eval_norm = _normalize(train_features, eval_features)
            _, risk_norm_eval = _normalize(train_risk, eval_risk)
            with torch.no_grad():
                scores = model(eval_norm, risk_norm_eval).squeeze(-1).numpy()
            for budget_pct in args.coverage_budgets:
                summary_frame, decision_frame = _evaluate_budget(
                    eval_meta,
                    scores,
                    budget_pct=budget_pct,
                    variant=variant,
    ...
    weighted_summary = (
        summary_df.assign(weight=summary_df["decisions"])
        .groupby(["variant", "budget_pct", "slice"], as_index=False)
        .apply(
            lambda frame: pd.Series(
                {
                    "decisions": frame["decisions"].sum(),
                    "coverage": np.average(frame["coverage"], weights=frame["weight"]),
                    "defer_precision": np.average(frame["defer_precision"], weights=frame["weight"]),
                    "false_positive_rate": np.average(frame["false_positive_rate"], weights=frame["weight"]),
                    "harmful_selection_rate": np.average(frame["harmful_selection_rate"], weights=frame["weight"]),
                    "correction_rate": np.average(frame["correction_rate"], weights=frame["weight"]),
                    "new_error_rate": np.average(frame["new_error_rate"], weights=frame["weight"]),
                    "base_target_match": np.average(frame["base_target_match"], weights=frame["weight"]),
                    "system_target_match": np.average(frame["system_target_match"], weights=frame["weight"]),
                    "mean_delta_regret": np.average(frame["mean_delta_regret"], weights=frame["weight"]),
                    "mean_delta_miss": np.average(frame["mean_delta_miss"], weights=frame["weight"]),
                }
            ),
    ...
    decision_path = output_prefix.with_name(output_prefix.name + "_decisions.csv")
    plot_path = output_prefix.with_name(output_prefix.name + "_summary.png")
    json_path = output_prefix.with_suffix(".json")

    weighted_summary.to_csv(summary_path, index=False)
    decision_df.to_csv(decision_path, index=False)
    _plot(weighted_summary, plot_path)
    json_path.write_text(
        json.dumps(
            {
                "variants": list(VARIANTS),
                "budgets": list(args.coverage_budgets),
