...
from gnn3.models.prototype_defer import SupportWeightedPrototypeMemoryDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_support_weighted_memory", "prototype_support_weighted_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_support_weighted_memory_defer",
...
def _fit_head(
    ...
    *,
    ...
    use_risk = variant != "prototype_support_weighted_memory"
    model = SupportWeightedPrototypeMemoryDeferHead(
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Support-Weighted Prototype-Memory Precision vs Coverage")
    axes[1].set_title("Support-Weighted Prototype-Memory Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        for split, eval_meta, eval_features, eval_risk in eval_sets:
            ...
            with torch.no_grad():
                scores = model(eval_norm, risk_norm_eval if variant != "prototype_support_weighted_memory" else None).squeeze(-1).numpy()
