...
from gnn3.models.prototype_defer import AdapterPrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_adapter", "prototype_adapter_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_adapter_defer",
...
def _fit_head(
    ...
    *,
    ...
    model = AdapterPrototypeDeferHead(
        ...
        use_risk_branch=variant == "prototype_adapter_hybrid",
    ).to(device)
    ...
    for _ in range(epochs):
        ...
        logits = model(x, risk if variant == "prototype_adapter_hybrid" else None)
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Prototype Adapter Stable-Positive Recovery")
    axes[1].set_title("Prototype Adapter Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        for split_name, eval_meta, eval_features, eval_risk in eval_payloads:
            ...
            with torch.no_grad():
                scores = model(eval_norm, risk_norm_eval if variant == "prototype_adapter_hybrid" else None).squeeze(-1).numpy()
