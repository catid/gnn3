...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_shared_teacher_rebuilt_negative_bank_branchwise_max_negative_cleanup_support_agreement_mixture_defer",
...
def _teacher_rebuild_shared_bank(
    *,
...
def _rebuild_model(
    base_model: SharedTeacherRebuiltNegativeBankBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead,
    ...
    *,
    ...
    with torch.no_grad():
        ...
        _, learned_shared_neg = base_model._shared_banks()
        harmful_weights = _teacher_candidate_weights(train_meta)
        rebuilt_shared, rebuilt_shared_support, stats = _teacher_rebuild_shared_bank(
            harmful_encoded=shared_negative_encoded,
            stable_encoded=shared_stable_encoded,
            harmful_weights=harmful_weights,
            learned_bank=learned_shared_neg,
            positive_overlap_penalty=args.positive_overlap_penalty,
        )

    rebuilt_model = SharedTeacherRebuiltNegativeBankBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
    ...
    rebuilt_model.set_rebuilt_shared_negative_bank(
        shared_negative_prototypes=rebuilt_shared,
        shared_negative_support=rebuilt_shared_support,
    )
    summary_rows = [
        {"variant": variant, "bank": "shared_negative", **stats},
...
def _plot(summary: pd.DataFrame, output_prefix: Path) -> None:
    ...
    ax.set_title("Shared teacher-rebuilt negative-bank branchwise-max support agreement mixture")
