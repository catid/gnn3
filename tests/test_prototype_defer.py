...
def test_teacher_rebuilt_negative_bank_branchwise_max_negative_cleanup_support_agreement_mixture_prototype_defer_rebuilds_and_separates() -> None:
    torch.manual_seed(128)
    positives = torch.randn(24, 2) * 0.15 + torch.tensor([1.0, 1.0])
    negatives = torch.randn(24, 2) * 0.15 + torch.tensor([-1.0, -1.0])
    features = torch.cat([positives, negatives], dim=0)
    ...
    head = TeacherRebuiltNegativeBankBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
        feature_dim=2,
        risk_dim=2,
        prototype_dim=4,
        positive_prototypes=4,
    ...
    shared_support = torch.tensor([0.8, 0.2, -0.1, -0.4], dtype=torch.float32)
    dual_support = torch.tensor([0.6, 0.3, -0.2, -0.5], dtype=torch.float32)
    head.set_rebuilt_negative_banks(
        shared_negative_prototypes=shared_bank,
        dual_negative_prototypes=dual_bank,
        shared_negative_support=shared_support,
        dual_negative_support=dual_support,
    )
    with torch.no_grad():
        assert torch.allclose(head.shared_negative_prototypes, F.normalize(shared_bank, dim=-1))
        assert torch.allclose(head.dual_negative_prototypes, F.normalize(dual_bank, dim=-1))
        assert torch.allclose(head.shared_negative_support, shared_support)
        assert torch.allclose(head.dual_negative_support, dual_support)
...
def test_shared_teacher_rebuilt_negative_bank_branchwise_max_negative_cleanup_support_agreement_mixture_prototype_defer_rebuilds_and_separates() -> None:
    torch.manual_seed(129)
    positives = torch.randn(24, 2) * 0.15 + torch.tensor([1.0, 1.0])
    negatives = torch.randn(24, 2) * 0.15 + torch.tensor([-1.0, -1.0])
    features = torch.cat([positives, negatives], dim=0)
    ...
    head = SharedTeacherRebuiltNegativeBankBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
        feature_dim=2,
        risk_dim=2,
        prototype_dim=4,
        positive_prototypes=4,
    ...
    shared_support = torch.tensor([0.7, 0.3, -0.2, -0.6], dtype=torch.float32)
    head.set_rebuilt_shared_negative_bank(
        shared_negative_prototypes=shared_bank,
        shared_negative_support=shared_support,
    )
    with torch.no_grad():
...
def test_dual_teacher_rebuilt_negative_bank_branchwise_max_negative_cleanup_support_agreement_mixture_prototype_defer_rebuilds_and_separates() -> None:
    torch.manual_seed(130)
    positives = torch.randn(24, 2) * 0.15 + torch.tensor([1.0, 1.0])
    negatives = torch.randn(24, 2) * 0.15 + torch.tensor([-1.0, -1.0])
    features = torch.cat([positives, negatives], dim=0)
    ...
    head = DualTeacherRebuiltNegativeBankBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
        feature_dim=2,
        risk_dim=2,
        prototype_dim=4,
        positive_prototypes=4,
    ...
    dual_support = torch.tensor([0.7, 0.2, -0.1, -0.5], dtype=torch.float32)
    head.set_rebuilt_dual_negative_bank(
        dual_negative_prototypes=dual_bank,
        dual_negative_support=dual_support,
    )
    with torch.no_grad():
...
def test_dual_teacher_rebuilt_negative_bank_rescue_weighted_anchored_dual_lift_branchwise_max_negative_cleanup_support_agreement_mixture_prototype_defer_rebuilds_and_separates() -> None:
    torch.manual_seed(131)
    positives = torch.randn(24, 2) * 0.15 + torch.tensor([1.0, 1.0])
    negatives = torch.randn(24, 2) * 0.15 + torch.tensor([-1.0, -1.0])
    features = torch.cat([positives, negatives], dim=0)
    ...
    head = DualTeacherRebuiltNegativeBankRescueWeightedAnchoredDualLiftBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
        feature_dim=2,
        risk_dim=2,
        prototype_dim=4,
        positive_prototypes=4,
    ...
    dual_support = torch.tensor([0.8, 0.3, -0.1, -0.4], dtype=torch.float32)
    head.set_rebuilt_dual_negative_bank(
        dual_negative_prototypes=dual_bank,
        dual_negative_support=dual_support,
    )
...
def test_gated_dual_teacher_rebuilt_negative_bank_rescue_weighted_anchored_dual_lift_branchwise_max_negative_cleanup_support_agreement_mixture_prototype_defer_blends_and_separates() -> None:
    torch.manual_seed(132)
    positives = torch.randn(24, 2) * 0.15 + torch.tensor([1.0, 1.0])
    negatives = torch.randn(24, 2) * 0.15 + torch.tensor([-1.0, -1.0])
    features = torch.cat([positives, negatives], dim=0)
    ...
    head = GatedDualTeacherRebuiltNegativeBankRescueWeightedAnchoredDualLiftBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
        feature_dim=2,
        risk_dim=2,
        prototype_dim=4,
        positive_prototypes=4,
    ...
    dual_support = torch.tensor([0.8, 0.3, -0.1, -0.4], dtype=torch.float32)
    with torch.no_grad():
        baseline_logits = head(features, risk)
    head.set_rebuilt_dual_negative_bank(
        dual_negative_prototypes=dual_bank,
        dual_negative_support=dual_support,
    )
    with torch.no_grad():
        head.dual_rebuild_blend_scale.zero_()
        head.dual_rebuild_blend_bias.fill_(10.0)
        rebuilt_logits = head(features, risk)
        assert torch.allclose(head.rebuilt_dual_negative_prototypes, F.normalize(dual_bank, dim=-1))
        assert torch.allclose(head.rebuilt_dual_negative_support, dual_support)
        assert not torch.allclose(rebuilt_logits, baseline_logits)
...
def test_max_dual_teacher_rebuilt_negative_bank_rescue_weighted_anchored_dual_lift_branchwise_max_negative_cleanup_support_agreement_mixture_prototype_defer_uses_rebuilt_bank_and_separates() -> None:
    ...
    head = MaxDualTeacherRebuiltNegativeBankRescueWeightedAnchoredDualLiftBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
        feature_dim=2,
        risk_dim=2,
        prototype_dim=4,
        positive_prototypes=4,
