...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-tail-gate", type=float, default=0.35)
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_floor_gated_sharp_negative_tail_support_agreement_mixture_defer",
...
def _fit_head(
    ...
    *,
    ...
    use_risk = variant == "prototype_floor_gated_sharp_negative_tail_support_agree_mix_hybrid"
    model = FloorGatedSharpNegativeTailSupportAgreementMixturePrototypeDeferHead(
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Floor-Gated Sharp Negative-Tail Precision vs Coverage")
    axes[1].set_title("Floor-Gated Sharp Negative-Tail Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    for variant in VARIANTS:
        model = _fit_head(
            ...
            min_tail_gate=args.min_tail_gate,
            device=device,
        )
        with torch.no_grad():
