...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--refit-epochs", type=int, default=120)
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_gated_dual_teacher_rebuilt_negative_bank_rescue_weighted_anchored_dual_lift_branchwise_max_negative_cleanup_support_agreement_mixture_defer",
...
def _fit_head(
    ...
    *,
    ...
    model = RescueWeightedAnchoredDualLiftBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
...
def _refit_blend_gate(
    model: GatedDualTeacherRebuiltNegativeBankRescueWeightedAnchoredDualLiftBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead,
    train_features: torch.Tensor,
    train_risk: torch.Tensor,
    target: torch.Tensor,
    weights: torch.Tensor,
    ...
    *,
    ...
    model.dual_rebuild_blend_scale.requires_grad_(True)
    model.dual_rebuild_blend_bias.requires_grad_(True)

    optimizer = torch.optim.AdamW(
        [model.dual_rebuild_blend_scale, model.dual_rebuild_blend_bias],
        lr=lr,
        weight_decay=0.0,
    )
    positive = target > 0.5
    positive_count = float(positive.sum().item())
    negative_count = float((~positive).sum().item())
    pos_weight = torch.tensor(negative_count / max(positive_count, 1.0), dtype=target.dtype)

    for _ in range(epochs):
        optimizer.zero_grad(set_to_none=True)
        logits = model(train_features, train_risk if use_risk else None)
        per_row = F.binary_cross_entropy_with_logits(logits, target, reduction="none", pos_weight=pos_weight)
        ...
        support_reg = model.support_regularization()
        tail_reg = model.tail_regularization()
        loss = (per_row * weights).mean() + 0.1 * reg + 0.01 * support_reg + 0.01 * tail_reg
        loss.backward()
        optimizer.step()
...
def _rebuild_model(
    base_model: RescueWeightedAnchoredDualLiftBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead,
    train_features: torch.Tensor,
    train_risk: torch.Tensor,
    train_target: torch.Tensor,
    train_weights: torch.Tensor,
    train_hard_negative: torch.Tensor,
    ...
    *,
    ...
    rebuilt_model = GatedDualTeacherRebuiltNegativeBankRescueWeightedAnchoredDualLiftBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
        feature_dim=train_features.size(1),
    ...
    *,
    ...
    for variant in VARIANTS:
        base_variant = (
            ...
            else "prototype_rescue_weighted_anchored_dual_lift_branchwise_max_negative_cleanup_support_agree_mix"
        )
        ...
        model, rebuild_frame = _rebuild_model(
            base_model,
            train_features_raw,
            train_risk,
            train_target,
...
def _plot(summary: pd.DataFrame, output_prefix: Path) -> None:
    ...
    ax.set_title("Gated dual teacher-rebuilt rescue-weighted anchored dual-lift branchwise-max")
