...
from gnn3.models.prototype_defer import BudgetConditionedEvidenceAgreementPrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_budget_evidence_agree", "prototype_budget_evidence_agree_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_budget_conditioned_evidence_agreement_defer",
...
def _budget_features(budget_pct: float, rows: int) -> torch.Tensor:
    normalized = float(budget_pct) / 2.0
    return torch.tensor(
        [[normalized, math.sqrt(max(normalized, 1e-8))]] * rows,
        dtype=torch.float32,
    )


def _expand_training(
    ...
    for budget_pct in budgets:
        budget_frac = float(budget_pct) / max(budgets)
        scaled = weights.clone()
        scaled[~positive_mask] = scaled[~positive_mask] * (1.0 + 2.0 * (1.0 - budget_frac))
        scaled[hard_negative_mask] = scaled[hard_negative_mask] * (1.0 + 1.5 * (1.0 - budget_frac))
        x_parts.append(train_features)
        risk_parts.append(train_risk)
        budget_parts.append(_budget_features(budget_pct, len(train_features)))
        y_parts.append(target)
        w_parts.append(scaled)
        hard_negative_parts.append(hard_negative_mask)
    return (
        torch.cat(x_parts, dim=0),
        torch.cat(risk_parts, dim=0),
        torch.cat(budget_parts, dim=0),
        torch.cat(y_parts, dim=0),
        torch.cat(w_parts, dim=0),
        torch.cat(hard_negative_parts, dim=0),
    )
...
def _fit_head(
    ...
    *,
    ...
    use_risk = variant == "prototype_budget_evidence_agree_hybrid"
    model = BudgetConditionedEvidenceAgreementPrototypeDeferHead(
    ...
    x, risk, budget_features, y, w, _ = [tensor.to(device) for tensor in expanded]
    base_x = train_features.to(device)
    base_hard_negative = hard_negative_mask.to(device)
    positive = target.to(device) > 0.5
    positive_count = float((y > 0.5).sum().item())
    negative_count = float((y <= 0.5).sum().item())
    pos_weight = torch.tensor(negative_count / max(positive_count, 1.0), device=device)
    for _ in range(epochs):
        optimizer.zero_grad(set_to_none=True)
        logits = model(x, budget_features, risk if use_risk else None)
        per_row = F.binary_cross_entropy_with_logits(logits, y, reduction="none", pos_weight=pos_weight)
        reg = model.regularization(base_x, positive_mask=positive, hard_negative_mask=base_hard_negative)
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Budget-Conditioned Evidence Precision vs Coverage")
    axes[1].set_title("Budget-Conditioned Evidence Hard Near-Tie Delta Regret")
    ...
    for variant in VARIANTS:
        ...
        for split_name, eval_meta, eval_features, eval_risk in eval_payloads:
            ...
            for budget_pct in args.coverage_budgets:
                budget_norm_eval = _budget_features(budget_pct, len(eval_norm))
                with torch.no_grad():
                    scores = model(
                        eval_norm,
                        budget_norm_eval,
                        risk_norm_eval if variant == "prototype_budget_evidence_agree_hybrid" else None,
                    ).numpy()
        ...
        payload.append(
            {
                "variant": variant,
                "use_risk_branch": variant == "prototype_budget_evidence_agree_hybrid",
                "positive_prototypes": args.positive_prototypes,
                "negative_prototypes": args.negative_prototypes,
                "prototype_dim": args.prototype_dim,
                "hidden_dim": args.hidden_dim,
                "coverage_budgets": list(args.coverage_budgets),
            }
        )
