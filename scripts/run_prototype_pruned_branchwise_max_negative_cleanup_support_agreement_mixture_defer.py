...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--keep-floor", type=float, default=0.05)
    parser.add_argument("--keep-regularization-weight", type=float, default=1e-2)
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_pruned_branchwise_max_negative_cleanup_support_agreement_mixture_defer",
...
def _fit_head(
    ...
    *,
    ...
    keep_floor: float,
    keep_regularization_weight: float,
    device: torch.device,
) -> PrunedBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead:
    use_risk = variant == "prototype_pruned_branchwise_max_negative_cleanup_support_agree_mix_hybrid"
    model = PrunedBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
    ...
    for _ in range(epochs):
        ...
        keep_reg = model.keep_regularization()
        loss = (
            (per_row * w).mean()
            + 0.1 * reg
            + 0.01 * support_reg
            + 0.01 * tail_reg
            + keep_regularization_weight * keep_reg
        )
        loss.backward()
        optimizer.step()
    return model.cpu()
    ...
    *,
    ...
    for variant in VARIANTS:
        model = _fit_head(
            ...
            keep_regularization_weight=args.keep_regularization_weight,
            device=device,
        )
        keep_rows.append({"variant": variant, **model.keep_summary()})
...
def _plot(summary: pd.DataFrame, output_prefix: Path) -> None:
    ...
    ax.set_title("Pruned branchwise-max negative-cleanup support agreement mixture")
...
def main() -> None:
    ...
    summary, decisions, keep_stats = _fit_and_score(train_cache, train_meta, eval_caches, eval_meta, args=args)
    ...
    output = {
        ...
        "keep_summary": keep_stats.to_dict(orient="records"),
    }
