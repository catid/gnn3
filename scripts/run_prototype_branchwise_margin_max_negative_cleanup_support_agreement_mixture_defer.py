...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--shared-branch-margin-init", type=float, default=0.2)
    parser.add_argument("--dual-branch-margin-init", type=float, default=0.2)
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_branchwise_margin_max_negative_cleanup_support_agreement_mixture_defer",
...
def _fit_head(
    ...
    *,
    ...
    shared_branch_margin_init: float,
    dual_branch_margin_init: float,
    device: torch.device,
) -> BranchwiseMarginMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead:
    use_risk = variant == "prototype_branchwise_margin_max_negative_cleanup_support_agree_mix_hybrid"
    model = BranchwiseMarginMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
        ...
        shared_branch_margin_init=shared_branch_margin_init,
        dual_branch_margin_init=dual_branch_margin_init,
    ).to(device)
...
def _plot_summary(summary: pd.DataFrame, output_path: Path) -> None:
    ...
    for variant in VARIANTS:
        frame = summary[(summary["variant"] == variant) & (summary["slice"] == "stable_positive_v2")].sort_values("budget_pct")
        axes[0].plot(frame["coverage"] * 100.0, frame["target_match_rate"] * 100.0, marker="o", label=variant)
    axes[0].set_xlabel("Overall coverage (%)")
    axes[0].set_ylabel("Stable-positive-v2 recovery (%)")
    axes[0].set_title("Sparse-positive recovery")
    axes[0].grid(True, alpha=0.3)
    axes[0].legend(fontsize=8)

    for variant in VARIANTS:
        frame = summary[(summary["variant"] == variant) & (summary["slice"] == "hard_near_tie")].sort_values("budget_pct")
        axes[1].plot(frame["coverage"] * 100.0, frame["target_match_rate"] * 100.0, marker="o", label=variant)
    axes[1].set_xlabel("Overall coverage (%)")
    axes[1].set_ylabel("Hard near-tie target match (%)")
    axes[1].set_title("Hard-slice match rate")
...
def main() -> None:
    ...
    if len(args.eval_caches) != len(args.eval_metadata):
        raise ValueError("Each eval cache requires a matching eval metadata CSV.")
    ...
    eval_payloads: list[tuple[str, pd.DataFrame, torch.Tensor, torch.Tensor]] = []
    for cache_path, meta_path in zip(args.eval_caches, args.eval_metadata, strict=True):
        ...
        eval_payloads.append((Path(cache_path).stem, eval_meta, eval_features, eval_risk))
    ...
    for variant in VARIANTS:
        model = _fit_head(
            ...
            shared_branch_margin_init=args.shared_branch_margin_init,
            dual_branch_margin_init=args.dual_branch_margin_init,
            device=device,
        )
        ...
        for split_name, eval_meta, eval_features, eval_risk in eval_payloads:
            ...
            del train_norm
            with torch.no_grad():
                scores = model(eval_norm, eval_risk if use_risk else None).numpy()
            for budget in args.coverage_budgets:
                summary_frame, decisions = _evaluate_budget(
                    eval_meta,
                    scores,
                    budget_pct=budget,
                    variant=variant,
    ...
    _plot_summary(summary, output_prefix.with_name(output_prefix.name + "_summary.png"))
    ...
    payload = {
        ...
        "decisions_csv": str(output_prefix.with_name(output_prefix.name + "_decisions.csv")),
        "summary_png": str(output_prefix.with_name(output_prefix.name + "_summary.png")),
    }
