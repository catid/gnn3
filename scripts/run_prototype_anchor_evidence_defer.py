...
from gnn3.models.prototype_defer import AnchoredEvidenceAgreementPrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_anchor_evidence", "prototype_anchor_evidence_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_anchor_evidence_defer",
...
def _fit_head(
    ...
    *,
    ...
    use_risk = variant == "prototype_anchor_evidence_hybrid"
    model = AnchoredEvidenceAgreementPrototypeDeferHead(
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Anchor-Evidence Precision vs Coverage")
    axes[1].set_title("Anchor-Evidence Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        for split_name, eval_meta, eval_features, eval_risk in eval_payloads:
            ...
            with torch.no_grad():
                scores = model(eval_norm, risk_norm_eval if variant == "prototype_anchor_evidence_hybrid" else None).numpy()
        ...
        payload.append(
            {
                "variant": variant,
                "use_risk_branch": variant == "prototype_anchor_evidence_hybrid",
                "positive_prototypes": args.positive_prototypes,
                "negative_prototypes": args.negative_prototypes,
                "prototype_dim": args.prototype_dim,
                "hidden_dim": args.hidden_dim,
