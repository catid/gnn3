...
from gnn3.models.prototype_defer import ResidualRegimeEvidenceAgreementPrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_residual_regime_evidence", "prototype_residual_regime_evidence_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_residual_regime_evidence_defer",
...
def _regime_targets(meta: pd.DataFrame, target: torch.Tensor) -> torch.Tensor:
    headroom = meta["high_headroom_near_tie_case"].to_numpy(copy=True)
    residual = meta["baseline_error_hard_near_tie_case"].to_numpy(copy=True)
    positive = target.numpy().astype(bool)
    regime = np.full(len(meta), -1, dtype=np.int64)
    regime[np.logical_and.reduce([positive, headroom, ~residual])] = 0
    regime[np.logical_and.reduce([positive, residual, ~headroom])] = 1
    return torch.as_tensor(regime, dtype=torch.long)
...
def _fit_head(
    ...
    *,
    ...
    use_risk = variant == "prototype_residual_regime_evidence_hybrid"
    model = ResidualRegimeEvidenceAgreementPrototypeDeferHead(
    ...
    y = target.to(device)
    regimes = regime_targets.to(device)
    w = weights.to(device)
    hard_negative = hard_negative_mask.to(device)
    positive = y > 0.5
    regime_mask = regimes >= 0
    ...
    for _ in range(epochs):
        ...
        if bool(regime_mask.any()):
            regime_loss = F.cross_entropy(regime_logits[regime_mask], regimes[regime_mask])
            loss = loss + 0.05 * regime_loss
        loss.backward()
        optimizer.step()
    return model.cpu()
...
def _evaluate_budget(
    ...
    *,
    ...
    for slice_name, mask in _slice_map(meta).items():
        ...
        idx = target.index.to_numpy(copy=True)
    ...
    decision_frame["combined_target_match"] = np.where(
        selected,
        meta["compute_target_match"].to_numpy(copy=True),
        meta["base_target_match"].to_numpy(copy=True),
    )
    decision_frame["delta_regret"] = np.where(selected, meta["delta_regret"].to_numpy(copy=True), 0.0)
    decision_frame["delta_miss"] = np.where(selected, meta["delta_miss"].to_numpy(copy=True), 0.0)
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Residual-Regime Evidence Precision vs Coverage")
    axes[1].set_title("Residual-Regime Evidence Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        regimes = _regime_targets(train_meta, target)
        ...
        for split_name, eval_meta, eval_features, eval_risk in eval_payloads:
            ...
            with torch.no_grad():
                scores = model(eval_norm, risk_norm_eval if variant == "prototype_residual_regime_evidence_hybrid" else None)
                scores = scores.numpy()
        ...
        payload.append(
            {
                "variant": variant,
                "use_risk_branch": variant == "prototype_residual_regime_evidence_hybrid",
                "positive_prototypes": args.positive_prototypes,
                "negative_prototypes": args.negative_prototypes,
                "prototype_dim": args.prototype_dim,
                "hidden_dim": args.hidden_dim,
