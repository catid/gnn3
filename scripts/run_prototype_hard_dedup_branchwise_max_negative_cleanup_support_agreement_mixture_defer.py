...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--negative-dedup-threshold", type=float, default=0.2)
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_hard_dedup_branchwise_max_negative_cleanup_support_agreement_mixture_defer",
...
def _fit_head(
    ...
    *,
    ...
    negative_dedup_threshold: float,
    device: torch.device,
) -> HardDedupBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead:
    use_risk = variant == "prototype_hard_dedup_branchwise_max_negative_cleanup_support_agree_mix_hybrid"
    model = HardDedupBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
        ...
        negative_dedup_threshold=negative_dedup_threshold,
    ).to(device)
    ...
    *,
    ...
    for variant in VARIANTS:
        model = _fit_head(
            ...
            negative_dedup_threshold=args.negative_dedup_threshold,
            device=device,
        )
        dedup_frames.append(pd.DataFrame([{"variant": variant, **model.dedup_summary()}]))
    ...
    return (
        ...
        pd.concat(dedup_frames, ignore_index=True),
    )
...
def _plot(summary: pd.DataFrame, output_prefix: Path) -> None:
    ...
    ax.set_title("Hard-dedup branchwise-max negative-cleanup support agreement mixture")
...
def main() -> None:
    ...
    summary, decisions, dedup_stats = _fit_and_score(train_cache, train_meta, eval_caches, eval_meta, args=args)
    ...
    dedup_stats.to_csv(output_prefix.with_name(output_prefix.name + "_dedup_stats.csv"), index=False)
    ...
    output = {
        ...
        "dedup_summary": dedup_stats.to_dict(orient="records"),
    }
