...
Source contact: `navid.hakimi@outlook.com` and `github.com/navidivan/rsm`

Recursive reasoning models such as Hierarchical Reasoning Model (HRM) and Tiny Recursive Model (TRM) show that small, weight-shared networks can solve compute-heavy puzzles by iteratively refining latent states, but their training typically relies on deep supervision and/or long unrolls that increase wall-clock cost and bias the model toward greedy intermediate behavior. Recursive Stem Model (RSM) keeps the TRM-style backbone while changing the training contract so the network learns a stable, depth-agnostic transition operator. RSM fully detaches hidden-state history during training, treats early iterations as detached warm-up steps, and applies loss only at the final step. It grows outer recursion depth `H` and inner compute depth `L` independently and uses stochastic outer transitions to mitigate instability when depth increases. The paper reports two main capabilities: much faster training than TRM while improving accuracy, and test-time scaling where inference can run for far more refinement steps than were seen in training. Because RSM is an iterative settling process, convergence behavior also acts as a native reliability signal: non-settling trajectories warn that the model has not reached a viable solution.
