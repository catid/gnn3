...
from gnn3.models.prototype_defer import AsymmetricTailSupportAgreementMixturePrototypeDeferHead

KEY_COLS = ["suite", "episode_index", "decision_index"]
VARIANTS = ("prototype_asymmetric_tail_support_agree_mix", "prototype_asymmetric_tail_support_agree_mix_hybrid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--positive-tail-shrink-scale", type=float, default=0.75)
    parser.add_argument("--negative-tail-shrink-scale", type=float, default=2.0)
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_asymmetric_tail_support_agreement_mixture_defer",
...
def _fit_head(
    ...
    *,
    ...
    hidden_dim: int,
    tail_margin: float,
    positive_tail_shrink_scale: float,
    negative_tail_shrink_scale: float,
    device: torch.device,
) -> AsymmetricTailSupportAgreementMixturePrototypeDeferHead:
    use_risk = variant == "prototype_asymmetric_tail_support_agree_mix_hybrid"
    model = AsymmetricTailSupportAgreementMixturePrototypeDeferHead(
        ...
        positive_tail_shrink_scale=positive_tail_shrink_scale,
        negative_tail_shrink_scale=negative_tail_shrink_scale,
    ).to(device)
...
def _plot(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("Asymmetric-Tail Agreement Precision vs Coverage")
    axes[1].set_title("Asymmetric-Tail Agreement Hard Near-Tie Delta Regret")
...
def main() -> None:
    ...
    eval_feature_blocks: list[torch.Tensor] = []
    eval_risk_blocks: list[torch.Tensor] = []
    for cache_path, meta_path in zip(args.eval_caches, args.eval_metadata, strict=True):
        ...
        eval_frames.append(meta)
        eval_feature_blocks.append(features)
        eval_risk_blocks.append(risk)

    eval_meta = pd.concat(eval_frames, ignore_index=True)
    eval_features = torch.cat(eval_feature_blocks, dim=0)
    eval_risk = torch.cat(eval_risk_blocks, dim=0)

    train_features, eval_features = _normalize(train_features, eval_features)
    train_risk, eval_risk = _normalize(train_risk, eval_risk)
    ...
    for variant in VARIANTS:
        model = _fit_head(
            ...
            positive_tail_shrink_scale=args.positive_tail_shrink_scale,
            negative_tail_shrink_scale=args.negative_tail_shrink_scale,
            device=device,
        )
        with torch.no_grad():
            scores = model(eval_features, eval_risk if variant.endswith("_hybrid") else None).numpy(force=True)
        metrics[variant] = {
        ...
        for budget in args.coverage_budgets:
            ...
            decision_parts.append(decisions_df)
