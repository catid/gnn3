...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--margin-grid", nargs="+", type=float, default=[0.2, 0.35, 0.5, 0.65, 0.8])
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_searched_fixed_margin_branchwise_max_negative_cleanup_support_agreement_mixture_defer",
...
def _search_margins(
    ...
    *,
    ...
    variant: str,
) -> tuple[FixedTailMarginBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead, pd.DataFrame]:
    use_risk = variant.endswith("_hybrid")
    search_model = FixedTailMarginBranchwiseMaxNegativeCleanupSupportAgreementMixturePrototypeDeferHead(
        feature_dim=train_features.size(1),
        risk_dim=train_risk.size(1),
        ...
        shared_fixed_margin=args.tail_margin,
        shared_sharp_margin=args.tail_margin,
        dual_fixed_margin=args.tail_margin,
        dual_sharp_margin=args.tail_margin,
    )
    search_model.load_state_dict(base_model.state_dict())
    search_model = search_model.cpu()
    train_features = train_features.cpu()
    train_risk = train_risk.cpu()

    search_rows: list[dict[str, object]] = []
    best_key: tuple[float, float, float, float, float, float] | None = None
    best_margins: dict[str, float] | None = None

    with torch.no_grad():
        for shared_fixed, shared_sharp, dual_fixed, dual_sharp in itertools.product(args.margin_grid, repeat=4):
            search_model.set_margins(
                shared_fixed_margin=shared_fixed,
                shared_sharp_margin=shared_sharp,
                dual_fixed_margin=dual_fixed,
                dual_sharp_margin=dual_sharp,
            )
    ...
    search_model.set_margins(**best_margins)
    ...
    return search_model, search_frame
...
def _fit_and_score(
    ...
    *,
    ...
    for variant in VARIANTS:
        ...
        model, search_frame = _search_margins(
            base_model,
            train_features,
            train_risk,
            train_meta,
        ...
        margin_frames.append(pd.DataFrame([{"variant": variant, **model.margin_summary()}]))
        use_risk = variant.endswith("_hybrid")
    ...
    return (
        ...
        pd.concat(margin_frames, ignore_index=True),
...
def _plot(summary: pd.DataFrame, output_prefix: Path) -> None:
    ...
    ax.set_title("Searched fixed-margin branchwise-max negative-cleanup support agreement mixture")
...
def main() -> None:
    ...
    summary, decisions, margin_stats, search_summary = _fit_and_score(train_cache, train_meta, eval_caches, eval_meta, args=args)
    ...
    margin_stats.to_csv(output_prefix.with_name(output_prefix.name + "_margin_stats.csv"), index=False)
    ...
    output = {
        ...
        "selected_margins": margin_stats.to_dict(orient="records"),
    }
