...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--shared-sharpness-center", type=float, default=0.75)
    parser.add_argument("--shared-sharpness-scale", type=float, default=4.0)
    parser.add_argument("--dual-sharpness-center", type=float, default=0.75)
    parser.add_argument("--dual-sharpness-scale", type=float, default=4.0)
    ...
    parser.add_argument(
        "--output-prefix",
        default="reports/plots/prototype_branch_calibrated_sharp_negative_tail_support_agreement_mixture_defer",
...
def _fit_head(
    ...
    *,
    ...
    shared_sharpness_center: float,
    shared_sharpness_scale: float,
    dual_sharpness_center: float,
    dual_sharpness_scale: float,
    device: torch.device,
) -> BranchCalibratedSharpNegativeTailSupportAgreementMixturePrototypeDeferHead:
    use_risk = variant == "prototype_branch_calibrated_sharp_negative_tail_support_agree_mix_hybrid"
    model = BranchCalibratedSharpNegativeTailSupportAgreementMixturePrototypeDeferHead(
        ...
        shared_sharpness_center=shared_sharpness_center,
        shared_sharpness_scale=shared_sharpness_scale,
        dual_sharpness_center=dual_sharpness_center,
        dual_sharpness_scale=dual_sharpness_scale,
    ).to(device)
    ...
    *,
    ...
    for variant in VARIANTS:
        model = _fit_head(
            ...
            shared_sharpness_center=args.shared_sharpness_center,
            shared_sharpness_scale=args.shared_sharpness_scale,
            dual_sharpness_center=args.dual_sharpness_center,
            dual_sharpness_scale=args.dual_sharpness_scale,
            device=device,
        )
    ...
    return pd.concat(summary_frames, ignore_index=True), pd.concat(decision_frames, ignore_index=True)
...
def _plot(summary: pd.DataFrame, output_prefix: Path) -> None:
    ...
    ax.set_title("Branch-calibrated sharp negative-tail support agreement mixture")
...
def main() -> None:
    ...
    summary, decisions = _fit_and_score(train_cache, train_meta, eval_caches, eval_meta, args=args)
